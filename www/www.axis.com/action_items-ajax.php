...
if (isset($_SERVER['HTTP_HOST']) && isset($_SERVER['PHP_SELF'])) {
	if (($_SERVER['HTTP_HOST'] == 'localdev.example.com') && is_int(strpos($_SERVER['PHP_SELF'], '/auto/'))) {
		require_once('action_items-functions.php');
	}
}
...
switch ($methodCall) {
	case 'createActionItem':
		...
		try {
			...
			if ($enforcePermissions) {
				...
				$arrRequiredOrPermissions = array(
					'action_items_manage' => 1
				);
				//$errorMessage = 'Permission denied.';
				//$errorMessage = 'Permission denied: cannot create new Action Item data values.';
				$arrErrorMessages = array(
					'Error creating: Action Item.<br>Permission Denied.' => 1
				);
			...
			if (!isset($formattedAttributeSubgroupName) || empty($formattedAttributeSubgroupName)) {
				$formattedAttributeSubgroupName = 'Action Items';
			}
			if (!isset($newAttributeGroupName) || empty($newAttributeGroupName)) {
				$newAttributeGroupName = 'manage-action_item-record';
			}

			if (!isset($get->project_id)) {
				$get->project_id = $project_id;
			}
			if (!isset($get->created_by_contact_id)) {
				$get->created_by_contact_id = $currentlyActiveContactId;
			}
			...
			// Retrieve all of the $_GET inputs automatically for the ActionItem record
			...
			$actionItem->setData($httpGetInputData);
		...
		if (!isset($primaryKeyAsString)) {
			$primaryKeyAsString = '';
			if (isset($actionItem) && $actionItem instanceof ActionItem) {
				$primaryKeyAsString = $actionItem->getPrimaryKeyAsString();
			}
		}
	...
	case 'loadActionItem':
		...
		try {
			...
			if ($enforcePermissions) {
				...
				$arrErrorMessages = array(
					'Error loading: Action Item.<br>Permission Denied.' => 1
				);
				require('code-generator/ajax-permissions.php');
			}
	...
	case 'updateActionItem':
		...
		try {
			...
			$message->reset($currentPhpScript);
			$errorMessage = 'Error updating: Action Item';
			$message->enqueueError($errorMessage, $currentPhpScript);
			...
			if ($enforcePermissions) {
				...
				//$errorMessage = 'Permission denied.';
				$arrErrorMessages = array(
					"Error updating Action Item - {$attributeName}.<br>Permission Denied." => 1
				);
				require('code-generator/ajax-permissions.php');
			}
			...
			$arrAllowableAttributes = array(
				...
				'action_item_type_id' => 'action item type id',
				'action_item_sequence_number' => 'action item sequence number',
				'action_item_status_id' => 'action item status id',
				'action_item_priority_id' => 'action item priority id',
				'action_item_cost_code_id' => 'action item cost code id',
				'created_by_contact_id' => 'created by contact id',
				'action_item_title' => 'action item title',
				'action_item' => 'action item',
				'created' => 'created',
				'action_item_due_date' => 'action item due date',
				'action_item_completed_timestamp' => 'action item completed timestamp',
			...
			if ($attributeSubgroupName == 'action_items') {
				...
				if ($actionItem) {
					...
					$existingValue = $actionItem->$attributeName;
					...
					$errorNumber = 1;
					$errorMessage = 'Action Item record does not exist.';
					$message->enqueueError($errorMessage, $currentPhpScript);
					throw new Exception($errorMessage);
				}
	...
	case 'updateAllActionItemAttributes':
		...
		try {
			...
			if ($enforcePermissions) {
				...
				//$errorMessage = 'Permission denied.';
				$arrErrorMessages = array(
					'Error updating Action Item.<br>Permission Denied.' => 1
				);
				require('code-generator/ajax-permissions.php');
			}
			...
			if ($attributeSubgroupName == 'action_items') {
				...
				if ($actionItem) {
					$existingData = $actionItem->getData();
					...
					if (!empty($data)) {
						...
						$errorNumber = 1;
						$errorMessage = 'Error updating: Action Item<br>No Changes In Values';
						$message->enqueueError($errorMessage, $currentPhpScript);
						$primaryKeyAsString = $actionItem->getPrimaryKeyAsString();
						throw new Exception($errorMessage);
					}
	...
	case 'deleteActionItem':
		...
		try {
			...
			if ($enforcePermissions) {
				...
				//$errorMessage = 'Permission denied.';
				$arrErrorMessages = array(
					'Error deleting Action Item.<br>Permission Denied.' => 1
				);
				require('code-generator/ajax-permissions.php');
			}
	...
	case 'saveActionItem':
		...
		try {
			...
			if ($enforcePermissions) {
				...
				$arrErrorMessages = array(
					'Error saving Action Item.<br>Permission Denied.' => 1
				);
				require('code-generator/ajax-permissions.php');
			}
			...
			$action_item_id = $actionItem->insertOnDuplicateKeyUpdate();
			if (isset($action_item_id) && !empty($action_item_id)) {
				$actionItem->action_item_id = $action_item_id;
				$actionItem->setId($action_item_id);
			}
