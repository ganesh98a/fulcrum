...
function isarchived(){	
	...
	var ajaxHandler = window.ajaxUrlPrefix  + 'impersonate-users-ajax.php?method=checkArchivedUser';
	...
	try {
		...
		var returnedJqXHR = $.ajax({
			...
			success: function(data)
			{
				...
				window.location = "/dashboard.php?pID="+btoa(encodeURIComponent(project_id));
...
ddaccordion.setCookie('expandable', resultIndex+'c');
...
ddaccordion2.init({ //top level headers initialization
	...
	animatedefault: false, //Should contents open by default be animated into view?
	persiststate: true, //persist state of opened contents within browser session?
	toggleclass: ["", "openheader2"], //Two CSS classes to be applied to the header when it's collapsed and expanded, respectively ["class1", "class2"]
	togglehtml: ["prefix", "", ""], //Additional HTML added to the header when it's collapsed and expanded, respectively ["position", "html1", "html2"] (see docs)
	animatespeed: "fast", //speed of animation: integer in milliseconds (ie: 200), or keywords "fast", "normal", or "slow"
	oninit:function(headers, expandedindices) { //custom code to run when headers have initalized
		//do nothing
	...
	onopenclose:function(header, index, state, isuseractivated) { //custom code to run whenever a header is opened or closed
		...
		if (isuseractivated && state == 'block') {
			if ($(header).next().children().length == 1) {
				var gotoURL = $(header).next().children().first().children().first().attr("href");
...
ddaccordion2.init({ //2nd level headers initialization
	headerclass: "moduleStyle", //Shared CSS class name of sub headers group that are expandable
	contentclass: "moduleFunctions", //Shared CSS class name of sub contents group
	revealtype: "click", //Reveal content when user clicks or onmouseover the header? Valid value: "click" or "mouseover
	mouseoverdelay: 200, //if revealtype="mouseover", set delay in milliseconds before header expands onMouseover
	collapseprev: true, //Collapse previous content (so only one open at any time)? true/false
	defaultexpanded: [], //index of content(s) open by default [index1, index2, etc]. [] denotes no content
...
var UrlVars = (function() {
	...
	function writeQueryStringVariablesWithObjectM(querystring,obj)
	{
		try {
	...
	function setmultiple(key, value)
	{
		...
		alert(querystring);
		obj[key] = value;
		writeQueryStringVariablesWithObjectM(querystring,obj);
	}
