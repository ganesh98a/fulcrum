...
var searchOrigin = document.location.origin;
var searchParams = new URLSearchParams(window.location.search);
var datapath = new Array();
if(searchParams.has('folder')){
	var getParams =searchParams.get('folder');
	var datapath=  getParams.split(',');
}

// datapath = ['liFolder_2','liFolder_21'];
// var datapath ='liFolder_2';
...
if(searchParams.has('folder')){
if(typeof datapath[0] !== "undefined" && datapath[0]!=''){
	if ($('#'+datapath[0]).length){
		var elementpath = document.getElementById(datapath[0]);
		if(datapath[0].indexOf("liFolder_") ==0 ){
			$('#'+datapath[0]).removeClass('collapsed').addClass('expanded');
			var file_manager_folder_id = datapath[0].replace('liFolder_', '');
			fileTreeParams.folderEvent=='click';
			var clickfn=1;
			showTree(elementpath, file_manager_folder_id);

		}else{
			var file_manager_folder_id = datapath[0].replace('li_', '');
			loadPreview(file_manager_folder_id, $(elementpath).parent().attr('class'));
		}
		var _searchedIndex = $.inArray(datapath[0],datapath);
		if(_searchedIndex >= 0){
			datapath.splice(_searchedIndex,1);
		}
	}else{
		messageAlert('you don\'t have permission to access this file, please contact admin or project manager.', 'errorMessage');
		return false;
	}
	

}else{
	fileTreeParams.folderEvent='';
}
}
}
...
function getLinkWindowSetup()
{
	$("#windowGetLink").dialog({
		autoOpen: false,
		height: 200,
		width: 650,
		modal: true,
...
function getLinkRightClickSelected(element)
{
	...
	if ( $(".ui-selected").length > 0) {
		...
		var arrMovedFileListItems = [];
			...
			$temp.remove();
			// var folderName = arrMovedFolderListItemsRel[arrMovedFolderListItemsRel.length-1].replaceAll('/','').toLowerCase();
			if(arrMovedFolderListItemsRel[arrMovedFolderListItemsRel.length-1].indexOf('/Plans/')>=0){
				var reUrl= '<a href="'+searchOrigin+'/modules-gc-budget-form.php?pID='+btoa(project_id)+'&type=email_bidder">Click here</a>';
				// messageAlert('Link copied. <b>Note:</b> This link can will only work for users with a MyFulcrum account who also have permissions to this location in File Manager.<br /> <p>Click', 'warningMessage');
				messageAlertFM('<b>Warning Message:</b> Link copied can only be accessed by users with registered Fulcrum account. '+reUrl+' to share the plans with external users.', 'warningMessage');
				
			}else{				
				messageAlertFM('Link copied. <b>Note:</b> This link can will only work for users with a MyFulcrum account who also have permissions to this location in File Manager.', 'successMessage');
			}
			
			// navigator.clipboard.writeText(getLink);
			// $("#linkPath").html(getLink);
			// $("#btnFolder").hide();
			// $("#windowGetLink").dialog('open');
}
}
 ...
 */
function messageAlertFM(text, messageStyle, labelStyle, elementId)
{
