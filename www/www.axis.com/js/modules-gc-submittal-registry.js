...
function loadImportCostCodesFromExcelSuccess(data, textStatus, jqXHR){
	try {
		...
		$("#divModalWindow").dialog({
			...
			width: 1000,
			modal: true,
			title: 'Import Submittal Registry',
			open: function() {
				$("body").addClass('noscroll');
			},
			close: function() {
...
function mapExcelHeaders(){
	...
	var filePath = $("#costCodeTemplate").val();
	var ajaxQueryString = "method=mapSubmittalExcelHeaders&filePath="+costCodeTemplate;
	var ajaxHandler = "modules-gc-budget-ajax.php";
	var returnedJqXHR = $.ajax({
		url: ajaxHandler,
		data: ajaxQueryString,
...
function importCostCodeExcel(divisionNumber,costcodeNumber,specificationNumber,submittalTitle,submittalNotes){
	var costCodeTemplate = $("#costCodeTemplate").val();
	...
	var ajaxQueryString = "method=uploadCostCodeSubmittalRegistry&filePath="+costCodeTemplate+
	"&divisionNumber="+divisionNumber+
	"&costcodeNumber="+costcodeNumber+
	"&specificationNumber="+specificationNumber+
	"&submittalTitle="+submittalTitle+
	"&submittalNotes="+submittalNotes;
	var ajaxHandler = "modules-gc-budget-ajax.php";
...
function downloadCostcodeTemplate(){
	var linktogenerate = 'xlsx/Import-Submittal-Registry-default-template.xlsx';
...
function mapCostcodeExcel(){
	...
	var specificationNumber = $('#specification_number').val();
	var submittalTitle= $('#submittal_title').val();
	var submittalNotes = $('#submittal_notes').val();

	if(!costcodeNumber || !specificationNumber || !submittalTitle){
		messageAlert('Please cost code, Specification number and Submittal title','errorMessage');
		return false;
	}
	...
	$.ajax({
		method:'GET',
		url:ajaxUrl,
		data:"method=checkSubmittalRegistryExist&filePath="+costCodeTemplate+
		"&costcodeNumber="+costcodeNumber+
		"&specificationNumber="+specificationNumber+
		"&submittalTitle="+submittalTitle+
		"&submittalNotes="+submittalNotes+
...
function gotoStepThree(){
	...
	if(!costcodeNumber){
		messageAlert('Please select cost code','errorMessage');
		return false;
	}
