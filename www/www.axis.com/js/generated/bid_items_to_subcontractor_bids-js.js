 ...
 */
function createBidItemToSubcontractorBid(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Bid Item To Subcontractor Bid';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Bid Items To Subcontractor Bids';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-bid_item_to_subcontractor_bid-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'bid_items_to_subcontractor_bids';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'bid_items_to_subcontractor_bids-ajax.php?method=createBidItemToSubcontractorBid';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildBidItemToSubcontractorBidHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadBidItemToSubcontractorBid(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_items_to_subcontractor_bids-ajax.php?method=loadBidItemToSubcontractorBid';
		}
		...
		var bid_item_to_subcontractor_bid_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--bid_item_to_subcontractor_bid_id--' + uniqueId;
		if ($("#" + bid_item_to_subcontractor_bid_id_element_id).length) {
			var bid_item_to_subcontractor_bid_id = $("#" + bid_item_to_subcontractor_bid_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&bid_item_to_subcontractor_bid_id=' + encodeURIComponent(bid_item_to_subcontractor_bid_id);
		}
...
function loadAllBidItemToSubcontractorBidRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_items_to_subcontractor_bids-ajax.php?method=loadAllBidItemToSubcontractorBidRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateBidItemToSubcontractorBid(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterBidItemToSubcontractorBidHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_items_to_subcontractor_bids-ajax.php?method=updateBidItemToSubcontractorBid';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllBidItemToSubcontractorBidAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_items_to_subcontractor_bids-ajax.php?method=updateAllBidItemToSubcontractorBidAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteBidItemToSubcontractorBid(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Bid Item To Subcontractor Bid?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_items_to_subcontractor_bids-ajax.php?method=deleteBidItemToSubcontractorBid';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveBidItemToSubcontractorBid(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_items_to_subcontractor_bids-ajax.php?method=saveBidItemToSubcontractorBid';
		}
...
function buildBidItemToSubcontractorBidHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + bid_item_to_subcontractor_bid_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					bid_item_to_subcontractor_bid_id = parseInputToMySQLUnsignedInt(bid_item_to_subcontractor_bid_id);
				}
				ajaxQueryString = ajaxQueryString + '&bid_item_to_subcontractor_bid_id=' + encodeURIComponent(bid_item_to_subcontractor_bid_id);
			}
		}

		var bid_item_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--bid_item_id--' + uniqueId;
		if ($("#" + bid_item_id_element_id).length) {
			var bid_item_id = $("#" + bid_item_id_element_id).val();
			bid_item_id = parseInputToInt(bid_item_id);
			ajaxQueryString = ajaxQueryString + '&bid_item_id=' + encodeURIComponent(bid_item_id);
		}
		...
		var item_quantity_element_id = attributeGroupName + '--' + attributeSubgroupName + '--item_quantity--' + uniqueId;
		if ($("#" + item_quantity_element_id).length) {
			var item_quantity = $("#" + item_quantity_element_id).val();
			item_quantity = parseInputToInt(item_quantity);
			ajaxQueryString = ajaxQueryString + '&item_quantity=' + encodeURIComponent(item_quantity);
		}
		var unit_element_id = attributeGroupName + '--' + attributeSubgroupName + '--unit--' + uniqueId;
		if ($("#" + unit_element_id).length) {
			var unit = $("#" + unit_element_id).val();
			ajaxQueryString = ajaxQueryString + '&unit=' + encodeURIComponent(unit);
		}
		var unit_price_element_id = attributeGroupName + '--' + attributeSubgroupName + '--unit_price--' + uniqueId;
		if ($("#" + unit_price_element_id).length) {
			var unit_price = $("#" + unit_price_element_id).val();
			unit_price = parseInputToCurrency(unit_price);
			ajaxQueryString = ajaxQueryString + '&unit_price=' + encodeURIComponent(unit_price);
		}
		var exclude_bid_item_flag_element_id = attributeGroupName + '--' + attributeSubgroupName + '--exclude_bid_item_flag--' + uniqueId;
		if ($("#" + exclude_bid_item_flag_element_id).length) {
			var exclude_bid_item_flag = $("#" + exclude_bid_item_flag_element_id).val();
			if ($("#" + exclude_bid_item_flag_element_id).is(':checkbox')) {
				if ($("#" + exclude_bid_item_flag_element_id).is(':checked')) {
					exclude_bid_item_flag = 'Y';
				} else {
					exclude_bid_item_flag = 'N';
				}
			}
			ajaxQueryString = ajaxQueryString + '&exclude_bid_item_flag=' + encodeURIComponent(exclude_bid_item_flag);
		}
...
function filterBidItemToSubcontractorBidHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'bid_item_to_subcontractor_bid_id':
