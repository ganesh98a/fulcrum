 ...
 */
function createActionItem(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Action Item';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Action Items';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-action_item-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'action_items';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'action_items-ajax.php?method=createActionItem';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildActionItemHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
		...
		$('.save_action_'+uniqueId).attr('disabled','disabled');
...
function loadActionItem(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'action_items-ajax.php?method=loadActionItem';
		}
...
function loadAllActionItemRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'action_items-ajax.php?method=loadAllActionItemRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateActionItem(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterActionItemHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'action_items-ajax.php?method=updateActionItem';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllActionItemAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'action_items-ajax.php?method=updateAllActionItemAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteActionItem(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Action Item?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'action_items-ajax.php?method=deleteActionItem';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveActionItem(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'action_items-ajax.php?method=saveActionItem';
		}
...
function buildActionItemHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		var action_item_type_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--action_item_type_id--' + uniqueId;
		if ($("#" + action_item_type_id_element_id).length) {
			var action_item_type_id = $("#" + action_item_type_id_element_id).val();
			action_item_type_id = parseInputToInt(action_item_type_id);
			ajaxQueryString = ajaxQueryString + '&action_item_type_id=' + encodeURIComponent(action_item_type_id);
		}

		var action_item_sequence_number_element_id = attributeGroupName + '--' + attributeSubgroupName + '--action_item_sequence_number--' + uniqueId;
		if ($("#" + action_item_sequence_number_element_id).length) {
			var action_item_sequence_number = $("#" + action_item_sequence_number_element_id).val();
			action_item_sequence_number = parseInputToInt(action_item_sequence_number);
			ajaxQueryString = ajaxQueryString + '&action_item_sequence_number=' + encodeURIComponent(action_item_sequence_number);
		}
		var action_item_status_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--action_item_status_id--' + uniqueId;
		if ($("#" + action_item_status_id_element_id).length) {
			var action_item_status_id = $("#" + action_item_status_id_element_id).val();
			action_item_status_id = parseInputToInt(action_item_status_id);
			ajaxQueryString = ajaxQueryString + '&action_item_status_id=' + encodeURIComponent(action_item_status_id);
		}

		var action_item_priority_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--action_item_priority_id--' + uniqueId;
		if ($("#" + action_item_priority_id_element_id).length) {
			var action_item_priority_id = $("#" + action_item_priority_id_element_id).val();
			action_item_priority_id = parseInputToInt(action_item_priority_id);
			ajaxQueryString = ajaxQueryString + '&action_item_priority_id=' + encodeURIComponent(action_item_priority_id);
		}

		var action_item_cost_code_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--action_item_cost_code_id--' + uniqueId;
		if ($("#" + action_item_cost_code_id_element_id).length) {
			var action_item_cost_code_id = $("#" + action_item_cost_code_id_element_id).val();
			action_item_cost_code_id = parseInputToInt(action_item_cost_code_id);
			ajaxQueryString = ajaxQueryString + '&action_item_cost_code_id=' + encodeURIComponent(action_item_cost_code_id);
		}
		...
		var action_item_title_element_id = attributeGroupName + '--' + attributeSubgroupName + '--action_item_title--' + uniqueId;
		if ($("#" + action_item_title_element_id).length) {
			var action_item_title = $("#" + action_item_title_element_id).val();
			ajaxQueryString = ajaxQueryString + '&action_item_title=' + encodeURIComponent(action_item_title);
		}
		var action_item_element_id = attributeGroupName + '--' + attributeSubgroupName + '--action_item--' + uniqueId;
		if ($("#" + action_item_element_id).length) {
			var action_item = $("#" + action_item_element_id).val();
			ajaxQueryString = ajaxQueryString + '&action_item=' + encodeURIComponent(action_item);
		}
		...
		var action_item_due_date_element_id = attributeGroupName + '--' + attributeSubgroupName + '--action_item_due_date--' + uniqueId;
		if ($("#" + action_item_due_date_element_id).length) {
			var action_item_due_date = $("#" + action_item_due_date_element_id).val();
			action_item_due_date = convertDateToMySQLFormat(action_item_due_date);
			ajaxQueryString = ajaxQueryString + '&action_item_due_date=' + encodeURIComponent(action_item_due_date);
		}
		var action_item_completed_timestamp_element_id = attributeGroupName + '--' + attributeSubgroupName + '--action_item_completed_timestamp--' + uniqueId;
		if ($("#" + action_item_completed_timestamp_element_id).length) {
			var action_item_completed_timestamp = $("#" + action_item_completed_timestamp_element_id).val();
			action_item_completed_timestamp = convertTimestampToMySQLFormat(action_item_completed_timestamp);
			ajaxQueryString = ajaxQueryString + '&action_item_completed_timestamp=' + encodeURIComponent(action_item_completed_timestamp);
		}
...
function filterActionItemHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	...
	switch (attributeName) {
		...
		case 'project_id':
			...
			inputDataFiltered = true;
			break;
		case 'action_item_type_id':
			var newValue = parseInputToMySQLUnsignedTinyInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'action_item_sequence_number':
			var newValue = parseInputToMySQLUnsignedSmallInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'action_item_status_id':
			var newValue = parseInputToMySQLUnsignedTinyInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'action_item_priority_id':
			var newValue = parseInputToMySQLUnsignedTinyInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'action_item_cost_code_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		...
		case 'created':
			...
			inputDataFiltered = true;
			break;
		case 'action_item_due_date':
			var newValue = convertDateToMySQLFormat(tmpValue);
			inputDataFiltered = true;
			break;
		case 'action_item_completed_timestamp':
			var newValue = convertTimestampToMySQLFormat(tmpValue);
			inputDataFiltered = true;
			break;
		case 'sort_order':
