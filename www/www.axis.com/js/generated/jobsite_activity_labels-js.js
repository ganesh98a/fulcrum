 ...
 */
function createJobsiteActivityLabel(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Jobsite Activity Label';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Jobsite Activity Labels';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-jobsite_activity_label-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'jobsite_activity_labels';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'jobsite_activity_labels-ajax.php?method=createJobsiteActivityLabel';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildJobsiteActivityLabelHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadJobsiteActivityLabel(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_activity_labels-ajax.php?method=loadJobsiteActivityLabel';
		}
		...
		var jobsite_activity_label_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_activity_label_id--' + uniqueId;
		if ($("#" + jobsite_activity_label_id_element_id).length) {
			var jobsite_activity_label_id = $("#" + jobsite_activity_label_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&jobsite_activity_label_id=' + encodeURIComponent(jobsite_activity_label_id);
		}
...
function loadAllJobsiteActivityLabelRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_activity_labels-ajax.php?method=loadAllJobsiteActivityLabelRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateJobsiteActivityLabel(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterJobsiteActivityLabelHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_activity_labels-ajax.php?method=updateJobsiteActivityLabel';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllJobsiteActivityLabelAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_activity_labels-ajax.php?method=updateAllJobsiteActivityLabelAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteJobsiteActivityLabel(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Jobsite Activity Label?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_activity_labels-ajax.php?method=deleteJobsiteActivityLabel';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveJobsiteActivityLabel(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_activity_labels-ajax.php?method=saveJobsiteActivityLabel';
		}
...
function buildJobsiteActivityLabelHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + jobsite_activity_label_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					jobsite_activity_label_id = parseInputToMySQLUnsignedInt(jobsite_activity_label_id);
				}
				ajaxQueryString = ajaxQueryString + '&jobsite_activity_label_id=' + encodeURIComponent(jobsite_activity_label_id);
			}
		}
		...
		var jobsite_activity_label_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_activity_label--' + uniqueId;
		if ($("#" + jobsite_activity_label_element_id).length) {
			var jobsite_activity_label = $("#" + jobsite_activity_label_element_id).val();
			ajaxQueryString = ajaxQueryString + '&jobsite_activity_label=' + encodeURIComponent(jobsite_activity_label);
		}
		var jobsite_activity_description_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_activity_description--' + uniqueId;
		if ($("#" + jobsite_activity_description_element_id).length) {
			var jobsite_activity_description = $("#" + jobsite_activity_description_element_id).val();
			ajaxQueryString = ajaxQueryString + '&jobsite_activity_description=' + encodeURIComponent(jobsite_activity_description);
		}
...
function filterJobsiteActivityLabelHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'jobsite_activity_label_id':
