 ...
 */
function createGcBudgetLineItemBidInvitation(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Gc Budget Line Item Bid Invitation';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Gc Budget Line Item Bid Invitations';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-gc_budget_line_item_bid_invitation-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'gc_budget_line_item_bid_invitations';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'gc_budget_line_item_bid_invitations-ajax.php?method=createGcBudgetLineItemBidInvitation';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildGcBudgetLineItemBidInvitationHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadGcBudgetLineItemBidInvitation(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_item_bid_invitations-ajax.php?method=loadGcBudgetLineItemBidInvitation';
		}
		...
		var gc_budget_line_item_bid_invitation_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--gc_budget_line_item_bid_invitation_id--' + uniqueId;
		if ($("#" + gc_budget_line_item_bid_invitation_id_element_id).length) {
			var gc_budget_line_item_bid_invitation_id = $("#" + gc_budget_line_item_bid_invitation_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&gc_budget_line_item_bid_invitation_id=' + encodeURIComponent(gc_budget_line_item_bid_invitation_id);
		}
...
function loadAllGcBudgetLineItemBidInvitationRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_item_bid_invitations-ajax.php?method=loadAllGcBudgetLineItemBidInvitationRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateGcBudgetLineItemBidInvitation(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterGcBudgetLineItemBidInvitationHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_item_bid_invitations-ajax.php?method=updateGcBudgetLineItemBidInvitation';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllGcBudgetLineItemBidInvitationAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_item_bid_invitations-ajax.php?method=updateAllGcBudgetLineItemBidInvitationAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteGcBudgetLineItemBidInvitation(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Gc Budget Line Item Bid Invitation?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_item_bid_invitations-ajax.php?method=deleteGcBudgetLineItemBidInvitation';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveGcBudgetLineItemBidInvitation(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_item_bid_invitations-ajax.php?method=saveGcBudgetLineItemBidInvitation';
		}
...
function buildGcBudgetLineItemBidInvitationHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + gc_budget_line_item_bid_invitation_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					gc_budget_line_item_bid_invitation_id = parseInputToMySQLUnsignedInt(gc_budget_line_item_bid_invitation_id);
				}
				ajaxQueryString = ajaxQueryString + '&gc_budget_line_item_bid_invitation_id=' + encodeURIComponent(gc_budget_line_item_bid_invitation_id);
			}
		}
		...
		var gc_budget_line_item_bid_invitation_sequence_number_element_id = attributeGroupName + '--' + attributeSubgroupName + '--gc_budget_line_item_bid_invitation_sequence_number--' + uniqueId;
		if ($("#" + gc_budget_line_item_bid_invitation_sequence_number_element_id).length) {
			var gc_budget_line_item_bid_invitation_sequence_number = $("#" + gc_budget_line_item_bid_invitation_sequence_number_element_id).val();
			gc_budget_line_item_bid_invitation_sequence_number = parseInputToInt(gc_budget_line_item_bid_invitation_sequence_number);
			ajaxQueryString = ajaxQueryString + '&gc_budget_line_item_bid_invitation_sequence_number=' + encodeURIComponent(gc_budget_line_item_bid_invitation_sequence_number);
		}
		var gc_budget_line_item_bid_invitation_file_manager_file_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--gc_budget_line_item_bid_invitation_file_manager_file_id--' + uniqueId;
		if ($("#" + gc_budget_line_item_bid_invitation_file_manager_file_id_element_id).length) {
			var gc_budget_line_item_bid_invitation_file_manager_file_id = $("#" + gc_budget_line_item_bid_invitation_file_manager_file_id_element_id).val();
			gc_budget_line_item_bid_invitation_file_manager_file_id = parseInputToInt(gc_budget_line_item_bid_invitation_file_manager_file_id);
			ajaxQueryString = ajaxQueryString + '&gc_budget_line_item_bid_invitation_file_manager_file_id=' + encodeURIComponent(gc_budget_line_item_bid_invitation_file_manager_file_id);
		}

		var gc_budget_line_item_bid_invitation_file_sha1_element_id = attributeGroupName + '--' + attributeSubgroupName + '--gc_budget_line_item_bid_invitation_file_sha1--' + uniqueId;
		if ($("#" + gc_budget_line_item_bid_invitation_file_sha1_element_id).length) {
			var gc_budget_line_item_bid_invitation_file_sha1 = $("#" + gc_budget_line_item_bid_invitation_file_sha1_element_id).val();
			ajaxQueryString = ajaxQueryString + '&gc_budget_line_item_bid_invitation_file_sha1=' + encodeURIComponent(gc_budget_line_item_bid_invitation_file_sha1);
		}
...
function filterGcBudgetLineItemBidInvitationHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'gc_budget_line_item_bid_invitation_id':
		...
		case 'gc_budget_line_item_id':
			...
			inputDataFiltered = true;
			break;
		case 'gc_budget_line_item_bid_invitation_sequence_number':
			var newValue = parseInputToMySQLUnsignedSmallInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'gc_budget_line_item_bid_invitation_file_manager_file_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'created':
