 ...
 */
function createChangeOrderResponseType(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Change Order Response Type';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Change Order Response Types';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-change_order_response_type-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'change_order_response_types';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'change_order_response_types-ajax.php?method=createChangeOrderResponseType';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildChangeOrderResponseTypeHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadChangeOrderResponseType(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_response_types-ajax.php?method=loadChangeOrderResponseType';
		}
		...
		var change_order_response_type_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--change_order_response_type_id--' + uniqueId;
		if ($("#" + change_order_response_type_id_element_id).length) {
			var change_order_response_type_id = $("#" + change_order_response_type_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&change_order_response_type_id=' + encodeURIComponent(change_order_response_type_id);
		}
...
function loadAllChangeOrderResponseTypeRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_response_types-ajax.php?method=loadAllChangeOrderResponseTypeRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateChangeOrderResponseType(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterChangeOrderResponseTypeHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_response_types-ajax.php?method=updateChangeOrderResponseType';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllChangeOrderResponseTypeAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_response_types-ajax.php?method=updateAllChangeOrderResponseTypeAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteChangeOrderResponseType(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Change Order Response Type?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_response_types-ajax.php?method=deleteChangeOrderResponseType';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveChangeOrderResponseType(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_response_types-ajax.php?method=saveChangeOrderResponseType';
		}
...
function buildChangeOrderResponseTypeHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + change_order_response_type_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					change_order_response_type_id = parseInputToMySQLUnsignedInt(change_order_response_type_id);
				}
				ajaxQueryString = ajaxQueryString + '&change_order_response_type_id=' + encodeURIComponent(change_order_response_type_id);
			}
		}

		var change_order_response_type_element_id = attributeGroupName + '--' + attributeSubgroupName + '--change_order_response_type--' + uniqueId;
		if ($("#" + change_order_response_type_element_id).length) {
			var change_order_response_type = $("#" + change_order_response_type_element_id).val();
			ajaxQueryString = ajaxQueryString + '&change_order_response_type=' + encodeURIComponent(change_order_response_type);
		}
...
function filterChangeOrderResponseTypeHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'change_order_response_type_id':
