 ...
 */
function createChangeOrderRecipient(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Change Order Recipient';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Change Order Recipients';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-change_order_recipient-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'change_order_recipients';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'change_order_recipients-ajax.php?method=createChangeOrderRecipient';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildChangeOrderRecipientHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadChangeOrderRecipient(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_recipients-ajax.php?method=loadChangeOrderRecipient';
		}
		...
		var change_order_notification_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--change_order_notification_id--' + uniqueId;
		if ($("#" + change_order_notification_id_element_id).length) {
			var change_order_notification_id = $("#" + change_order_notification_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&change_order_notification_id=' + encodeURIComponent(change_order_notification_id);
		}
		var co_additional_recipient_contact_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--co_additional_recipient_contact_id--' + uniqueId;
		if ($("#" + co_additional_recipient_contact_id_element_id).length) {
			var co_additional_recipient_contact_id = $("#" + co_additional_recipient_contact_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&co_additional_recipient_contact_id=' + encodeURIComponent(co_additional_recipient_contact_id);
		}
...
function loadAllChangeOrderRecipientRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_recipients-ajax.php?method=loadAllChangeOrderRecipientRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateChangeOrderRecipient(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterChangeOrderRecipientHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_recipients-ajax.php?method=updateChangeOrderRecipient';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllChangeOrderRecipientAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_recipients-ajax.php?method=updateAllChangeOrderRecipientAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteChangeOrderRecipient(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Change Order Recipient?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_recipients-ajax.php?method=deleteChangeOrderRecipient';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveChangeOrderRecipient(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_recipients-ajax.php?method=saveChangeOrderRecipient';
		}
...
function buildChangeOrderRecipientHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if ($("#" + change_order_notification_id_element_id).length) {
			var change_order_notification_id = $("#" + change_order_notification_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					change_order_notification_id = parseInputToMySQLUnsignedInt(change_order_notification_id);
				}
			ajaxQueryString = ajaxQueryString + '&change_order_notification_id=' + encodeURIComponent(change_order_notification_id);
		}
		...
		if ($("#" + co_additional_recipient_contact_id_element_id).length) {
			var co_additional_recipient_contact_id = $("#" + co_additional_recipient_contact_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					co_additional_recipient_contact_id = parseInputToMySQLUnsignedInt(co_additional_recipient_contact_id);
				}
			ajaxQueryString = ajaxQueryString + '&co_additional_recipient_contact_id=' + encodeURIComponent(co_additional_recipient_contact_id);
		}

		var co_additional_recipient_contact_mobile_phone_number_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--co_additional_recipient_contact_mobile_phone_number_id--' + uniqueId;
		if ($("#" + co_additional_recipient_contact_mobile_phone_number_id_element_id).length) {
			var co_additional_recipient_contact_mobile_phone_number_id = $("#" + co_additional_recipient_contact_mobile_phone_number_id_element_id).val();
			co_additional_recipient_contact_mobile_phone_number_id = parseInputToInt(co_additional_recipient_contact_mobile_phone_number_id);
			ajaxQueryString = ajaxQueryString + '&co_additional_recipient_contact_mobile_phone_number_id=' + encodeURIComponent(co_additional_recipient_contact_mobile_phone_number_id);
		}
...
function filterChangeOrderRecipientHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'change_order_notification_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'co_additional_recipient_contact_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'co_additional_recipient_contact_mobile_phone_number_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		default:
