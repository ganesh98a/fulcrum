 ...
 */
function createMeeting(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Meeting';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Meetings';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-meeting-record';
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildMeetingHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadMeeting(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'meetings-ajax.php?method=loadMeeting';
		}
...
function loadAllMeetingRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'meetings-ajax.php?method=loadAllMeetingRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateMeeting(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterMeetingHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'meetings-ajax.php?method=updateMeeting';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllMeetingAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'meetings-ajax.php?method=updateAllMeetingAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteMeeting(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Meeting?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'meetings-ajax.php?method=deleteMeeting';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveMeeting(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'meetings-ajax.php?method=saveMeeting';
		}
...
function buildMeetingHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		var previous_meeting_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--previous_meeting_id--' + uniqueId;
		if ($("#" + previous_meeting_id_element_id).length) {
			var previous_meeting_id = $("#" + previous_meeting_id_element_id).val();
			previous_meeting_id = parseInputToInt(previous_meeting_id);
			ajaxQueryString = ajaxQueryString + '&previous_meeting_id=' + encodeURIComponent(previous_meeting_id);
		}
		...
		if ($("#" + meeting_type_id_element_id).length) {
			...
			meeting_type_id = parseInputToInt(meeting_type_id);
			ajaxQueryString = ajaxQueryString + '&meeting_type_id=' + encodeURIComponent(meeting_type_id);
		}

		var meeting_location_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--meeting_location_id--' + uniqueId;
		if ($("#" + meeting_location_id_element_id).length) {
			var meeting_location_id = $("#" + meeting_location_id_element_id).val();
			meeting_location_id = parseInputToInt(meeting_location_id);
			ajaxQueryString = ajaxQueryString + '&meeting_location_id=' + encodeURIComponent(meeting_location_id);
		}

		var meeting_chair_contact_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--meeting_chair_contact_id--' + uniqueId;
		if ($("#" + meeting_chair_contact_id_element_id).length) {
			var meeting_chair_contact_id = $("#" + meeting_chair_contact_id_element_id).val();
			meeting_chair_contact_id = parseInputToInt(meeting_chair_contact_id);
			ajaxQueryString = ajaxQueryString + '&meeting_chair_contact_id=' + encodeURIComponent(meeting_chair_contact_id);
		}

		var modified_by_contact_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--modified_by_contact_id--' + uniqueId;
		if ($("#" + modified_by_contact_id_element_id).length) {
			var modified_by_contact_id = $("#" + modified_by_contact_id_element_id).val();
			modified_by_contact_id = parseInputToInt(modified_by_contact_id);
			ajaxQueryString = ajaxQueryString + '&modified_by_contact_id=' + encodeURIComponent(modified_by_contact_id);
		}

		var meeting_sequence_number_element_id = attributeGroupName + '--' + attributeSubgroupName + '--meeting_sequence_number--' + uniqueId;
		if ($("#" + meeting_sequence_number_element_id).length) {
			var meeting_sequence_number = $("#" + meeting_sequence_number_element_id).val();
			ajaxQueryString = ajaxQueryString + '&meeting_sequence_number=' + encodeURIComponent(meeting_sequence_number);
		}
		var meeting_start_date_element_id = attributeGroupName + '--' + attributeSubgroupName + '--meeting_start_date--' + uniqueId;
		if ($("#" + meeting_start_date_element_id).length) {
			...
			ajaxQueryString = ajaxQueryString + '&meeting_start_date=' + encodeURIComponent(meeting_start_date);
		}
		var meeting_start_time_element_id = attributeGroupName + '--' + attributeSubgroupName + '--meeting_start_time--' + uniqueId;
		if ($("#" + meeting_start_time_element_id).length) {
			...
			ajaxQueryString = ajaxQueryString + '&meeting_start_time=' + encodeURIComponent(meeting_start_time);
		}
		var meeting_end_date_element_id = attributeGroupName + '--' + attributeSubgroupName + '--meeting_end_date--' + uniqueId;
		if ($("#" + meeting_end_date_element_id).length) {
			...
			ajaxQueryString = ajaxQueryString + '&meeting_end_date=' + encodeURIComponent(meeting_end_date);
		}
		var meeting_end_time_element_id = attributeGroupName + '--' + attributeSubgroupName + '--meeting_end_time--' + uniqueId;
		if ($("#" + meeting_end_time_element_id).length) {
			...
			ajaxQueryString = ajaxQueryString + '&meeting_end_time=' + encodeURIComponent(meeting_end_time);
		}
		...
		var all_day_event_flag_element_id = attributeGroupName + '--' + attributeSubgroupName + '--all_day_event_flag--' + uniqueId;
		if ($("#" + all_day_event_flag_element_id).length) {
			var all_day_event_flag = $("#" + all_day_event_flag_element_id).val();
			if ($("#" + all_day_event_flag_element_id).is(':checkbox')) {
				if ($("#" + all_day_event_flag_element_id).is(':checked')) {
					all_day_event_flag = 'Y';
				} else {
					all_day_event_flag = 'N';
				}
			}
			ajaxQueryString = ajaxQueryString + '&all_day_event_flag=' + encodeURIComponent(all_day_event_flag);
		}
...
function filterMeetingHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	...
	switch (attributeName) {
		...
		case 'meeting_location_id':
			...
			inputDataFiltered = true;
			break;
		case 'meeting_chair_contact_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'modified_by_contact_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		...
		case 'meeting_start_time':
			var newValue = convertTimeToMySQLFormat(tmpValue);
			inputDataFiltered = true;
			break;
...
function submittal_data()
{
	$('#submittal_value').toggle();
}
function rfi_data()
{
	$('#rfi_value').toggle();
