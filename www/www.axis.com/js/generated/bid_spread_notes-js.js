 ...
 */
function createBidSpreadNote(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Bid Spread Note';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Bid Spread Notes';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-bid_spread_note-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'bid_spread_notes';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'bid_spread_notes-ajax.php?method=createBidSpreadNote';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildBidSpreadNoteHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadBidSpreadNote(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_spread_notes-ajax.php?method=loadBidSpreadNote';
		}
		...
		var bid_spread_note_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--bid_spread_note_id--' + uniqueId;
		if ($("#" + bid_spread_note_id_element_id).length) {
			var bid_spread_note_id = $("#" + bid_spread_note_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&bid_spread_note_id=' + encodeURIComponent(bid_spread_note_id);
		}
...
function loadAllBidSpreadNoteRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_spread_notes-ajax.php?method=loadAllBidSpreadNoteRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateBidSpreadNote(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterBidSpreadNoteHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_spread_notes-ajax.php?method=updateBidSpreadNote';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllBidSpreadNoteAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_spread_notes-ajax.php?method=updateAllBidSpreadNoteAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteBidSpreadNote(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Bid Spread Note?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_spread_notes-ajax.php?method=deleteBidSpreadNote';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveBidSpreadNote(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'bid_spread_notes-ajax.php?method=saveBidSpreadNote';
		}
...
function buildBidSpreadNoteHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + bid_spread_note_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					bid_spread_note_id = parseInputToMySQLUnsignedInt(bid_spread_note_id);
				}
				ajaxQueryString = ajaxQueryString + '&bid_spread_note_id=' + encodeURIComponent(bid_spread_note_id);
			}
		}

		var bid_spread_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--bid_spread_id--' + uniqueId;
		if ($("#" + bid_spread_id_element_id).length) {
			var bid_spread_id = $("#" + bid_spread_id_element_id).val();
			bid_spread_id = parseInputToInt(bid_spread_id);
			ajaxQueryString = ajaxQueryString + '&bid_spread_id=' + encodeURIComponent(bid_spread_id);
		}
		...
		var bid_spread_note_element_id = attributeGroupName + '--' + attributeSubgroupName + '--bid_spread_note--' + uniqueId;
		if ($("#" + bid_spread_note_element_id).length) {
			var bid_spread_note = $("#" + bid_spread_note_element_id).val();
			ajaxQueryString = ajaxQueryString + '&bid_spread_note=' + encodeURIComponent(bid_spread_note);
		}
...
function filterBidSpreadNoteHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'bid_spread_note_id':
