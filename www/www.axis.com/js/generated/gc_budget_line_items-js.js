 ...
 */
function createGcBudgetLineItem(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Gc Budget Line Item';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Gc Budget Line Items';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-gc_budget_line_item-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'gc_budget_line_items';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'gc_budget_line_items-ajax.php?method=createGcBudgetLineItem';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildGcBudgetLineItemHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadGcBudgetLineItem(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_items-ajax.php?method=loadGcBudgetLineItem';
		}
...
function loadAllGcBudgetLineItemRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_items-ajax.php?method=loadAllGcBudgetLineItemRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
 function updateGcBudgetLineItem(element, options, calVal)
 {
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterGcBudgetLineItemHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_items-ajax.php?method=updateGcBudgetLineItem';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllGcBudgetLineItemAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_items-ajax.php?method=updateAllGcBudgetLineItemAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteGcBudgetLineItem(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Gc Budget Line Item?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_items-ajax.php?method=deleteGcBudgetLineItem';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveGcBudgetLineItem(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'gc_budget_line_items-ajax.php?method=saveGcBudgetLineItem';
		}
...
function buildGcBudgetLineItemHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + gc_budget_line_item_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					gc_budget_line_item_id = parseInputToMySQLUnsignedInt(gc_budget_line_item_id);
				}
				ajaxQueryString = ajaxQueryString + '&gc_budget_line_item_id=' + encodeURIComponent(gc_budget_line_item_id);
			}
		}
		...
		var prime_contract_scheduled_value_element_id = attributeGroupName + '--' + attributeSubgroupName + '--prime_contract_scheduled_value--' + uniqueId;
		if ($("#" + prime_contract_scheduled_value_element_id).length) {
			var prime_contract_scheduled_value = $("#" + prime_contract_scheduled_value_element_id).val();
			prime_contract_scheduled_value = parseInputToCurrency(prime_contract_scheduled_value);
			ajaxQueryString = ajaxQueryString + '&prime_contract_scheduled_value=' + encodeURIComponent(prime_contract_scheduled_value);
		}
		var forecasted_expenses_element_id = attributeGroupName + '--' + attributeSubgroupName + '--forecasted_expenses--' + uniqueId;
		if ($("#" + forecasted_expenses_element_id).length) {
			var forecasted_expenses = $("#" + forecasted_expenses_element_id).val();
			forecasted_expenses = parseInputToCurrency(forecasted_expenses);
			ajaxQueryString = ajaxQueryString + '&forecasted_expenses=' + encodeURIComponent(forecasted_expenses);
		}

		var buyout_forecasted_expenses_element_id = attributeGroupName + '--' + attributeSubgroupName + '--buyout_forecasted_expenses--' + uniqueId;
		if ($("#" + buyout_forecasted_expenses_element_id).length) {
			var buyout_forecasted_expenses = $("#" + buyout_forecasted_expenses_element_id).val();
			buyout_forecasted_expenses = parseInputToCurrency(buyout_forecasted_expenses);
			ajaxQueryString = ajaxQueryString + '&buyout_forecasted_expenses=' + encodeURIComponent(buyout_forecasted_expenses);
		}
...
function filterGcBudgetLineItemHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	...
	switch (attributeName) {
		...
		case 'modified':
			...
			inputDataFiltered = true;
			break;
		case 'prime_contract_scheduled_value':
			var newValue = parseInputToCurrency(tmpValue);
			inputDataFiltered = true;
			break;
		case 'forecasted_expenses':
			var newValue = parseInputToCurrency(tmpValue);
			inputDataFiltered = true;
			break;
		case 'buyout_forecasted_expenses':
			var newValue = parseInputToCurrency(tmpValue);
			inputDataFiltered = true;
			break;
		case 'created':
