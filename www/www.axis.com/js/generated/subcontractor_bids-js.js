 ...
 */
function createSubcontractorBid(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Subcontractor Bid';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Subcontractor Bids';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-subcontractor_bid-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'subcontractor_bids';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'subcontractor_bids-ajax.php?method=createSubcontractorBid';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildSubcontractorBidHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadSubcontractorBid(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bids-ajax.php?method=loadSubcontractorBid';
		}
...
function loadAllSubcontractorBidRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bids-ajax.php?method=loadAllSubcontractorBidRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateSubcontractorBid(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterSubcontractorBidHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bids-ajax.php?method=updateSubcontractorBid';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllSubcontractorBidAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bids-ajax.php?method=updateAllSubcontractorBidAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteSubcontractorBid(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Subcontractor Bid?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bids-ajax.php?method=deleteSubcontractorBid';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveSubcontractorBid(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bids-ajax.php?method=saveSubcontractorBid';
		}
...
function buildSubcontractorBidHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + subcontractor_bid_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					subcontractor_bid_id = parseInputToMySQLUnsignedInt(subcontractor_bid_id);
				}
				ajaxQueryString = ajaxQueryString + '&subcontractor_bid_id=' + encodeURIComponent(subcontractor_bid_id);
			}
		}
		...
		var subcontractor_contact_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--subcontractor_contact_id--' + uniqueId;
		if ($("#" + subcontractor_contact_id_element_id).length) {
			var subcontractor_contact_id = $("#" + subcontractor_contact_id_element_id).val();
			subcontractor_contact_id = parseInputToInt(subcontractor_contact_id);
			ajaxQueryString = ajaxQueryString + '&subcontractor_contact_id=' + encodeURIComponent(subcontractor_contact_id);
		}

		var subcontractor_bid_status_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--subcontractor_bid_status_id--' + uniqueId;
		if ($("#" + subcontractor_bid_status_id_element_id).length) {
			var subcontractor_bid_status_id = $("#" + subcontractor_bid_status_id_element_id).val();
			subcontractor_bid_status_id = parseInputToInt(subcontractor_bid_status_id);
			ajaxQueryString = ajaxQueryString + '&subcontractor_bid_status_id=' + encodeURIComponent(subcontractor_bid_status_id);
		}
...
function filterSubcontractorBidHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	...
	switch (attributeName) {
		...
		case 'gc_budget_line_item_id':
			...
			inputDataFiltered = true;
			break;
		case 'subcontractor_contact_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'subcontractor_bid_status_id':
