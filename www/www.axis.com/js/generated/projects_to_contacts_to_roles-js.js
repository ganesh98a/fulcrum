 ...
 */
function createProjectToContactToRole(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Project To Contact To Role';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Projects To Contacts To Roles';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-project_to_contact_to_role-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'projects_to_contacts_to_roles';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'projects_to_contacts_to_roles-ajax.php?method=createProjectToContactToRole';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildProjectToContactToRoleHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadProjectToContactToRole(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'projects_to_contacts_to_roles-ajax.php?method=loadProjectToContactToRole';
		}
		...
		var contact_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--contact_id--' + uniqueId;
		if ($("#" + contact_id_element_id).length) {
			var contact_id = $("#" + contact_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&contact_id=' + encodeURIComponent(contact_id);
		}
		var role_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--role_id--' + uniqueId;
		if ($("#" + role_id_element_id).length) {
			var role_id = $("#" + role_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&role_id=' + encodeURIComponent(role_id);
		}
...
function loadAllProjectToContactToRoleRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'projects_to_contacts_to_roles-ajax.php?method=loadAllProjectToContactToRoleRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateProjectToContactToRole(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterProjectToContactToRoleHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'projects_to_contacts_to_roles-ajax.php?method=updateProjectToContactToRole';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllProjectToContactToRoleAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'projects_to_contacts_to_roles-ajax.php?method=updateAllProjectToContactToRoleAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteProjectToContactToRole(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Project To Contact To Role?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'projects_to_contacts_to_roles-ajax.php?method=deleteProjectToContactToRole';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveProjectToContactToRole(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'projects_to_contacts_to_roles-ajax.php?method=saveProjectToContactToRole';
		}
...
function buildProjectToContactToRoleHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if ($("#" + contact_id_element_id).length) {
			var contact_id = $("#" + contact_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					contact_id = parseInputToMySQLUnsignedInt(contact_id);
				}
			ajaxQueryString = ajaxQueryString + '&contact_id=' + encodeURIComponent(contact_id);
		}
		...
		if ($("#" + role_id_element_id).length) {
			var role_id = $("#" + role_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					role_id = parseInputToMySQLUnsignedInt(role_id);
				}
			ajaxQueryString = ajaxQueryString + '&role_id=' + encodeURIComponent(role_id);
		}
...
function filterProjectToContactToRoleHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
