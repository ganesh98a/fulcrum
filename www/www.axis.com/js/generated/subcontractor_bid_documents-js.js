 ...
 */
function createSubcontractorBidDocument(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Subcontractor Bid Document';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Subcontractor Bid Documents';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-subcontractor_bid_document-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'subcontractor_bid_documents';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'subcontractor_bid_documents-ajax.php?method=createSubcontractorBidDocument';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildSubcontractorBidDocumentHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadSubcontractorBidDocument(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bid_documents-ajax.php?method=loadSubcontractorBidDocument';
		}
		...
		var subcontractor_bid_document_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--subcontractor_bid_document_id--' + uniqueId;
		if ($("#" + subcontractor_bid_document_id_element_id).length) {
			var subcontractor_bid_document_id = $("#" + subcontractor_bid_document_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&subcontractor_bid_document_id=' + encodeURIComponent(subcontractor_bid_document_id);
		}
...
function loadAllSubcontractorBidDocumentRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bid_documents-ajax.php?method=loadAllSubcontractorBidDocumentRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateSubcontractorBidDocument(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterSubcontractorBidDocumentHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bid_documents-ajax.php?method=updateSubcontractorBidDocument';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllSubcontractorBidDocumentAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bid_documents-ajax.php?method=updateAllSubcontractorBidDocumentAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteSubcontractorBidDocument(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Subcontractor Bid Document?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bid_documents-ajax.php?method=deleteSubcontractorBidDocument';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveSubcontractorBidDocument(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'subcontractor_bid_documents-ajax.php?method=saveSubcontractorBidDocument';
		}
...
function buildSubcontractorBidDocumentHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + subcontractor_bid_document_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					subcontractor_bid_document_id = parseInputToMySQLUnsignedInt(subcontractor_bid_document_id);
				}
				ajaxQueryString = ajaxQueryString + '&subcontractor_bid_document_id=' + encodeURIComponent(subcontractor_bid_document_id);
			}
		}
		...
		var subcontractor_bid_document_type_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--subcontractor_bid_document_type_id--' + uniqueId;
		if ($("#" + subcontractor_bid_document_type_id_element_id).length) {
			var subcontractor_bid_document_type_id = $("#" + subcontractor_bid_document_type_id_element_id).val();
			subcontractor_bid_document_type_id = parseInputToInt(subcontractor_bid_document_type_id);
			ajaxQueryString = ajaxQueryString + '&subcontractor_bid_document_type_id=' + encodeURIComponent(subcontractor_bid_document_type_id);
		}

		var subcontractor_bid_document_sequence_number_element_id = attributeGroupName + '--' + attributeSubgroupName + '--subcontractor_bid_document_sequence_number--' + uniqueId;
		if ($("#" + subcontractor_bid_document_sequence_number_element_id).length) {
			var subcontractor_bid_document_sequence_number = $("#" + subcontractor_bid_document_sequence_number_element_id).val();
			subcontractor_bid_document_sequence_number = parseInputToInt(subcontractor_bid_document_sequence_number);
			ajaxQueryString = ajaxQueryString + '&subcontractor_bid_document_sequence_number=' + encodeURIComponent(subcontractor_bid_document_sequence_number);
		}
		var subcontractor_bid_document_file_manager_file_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--subcontractor_bid_document_file_manager_file_id--' + uniqueId;
		if ($("#" + subcontractor_bid_document_file_manager_file_id_element_id).length) {
			var subcontractor_bid_document_file_manager_file_id = $("#" + subcontractor_bid_document_file_manager_file_id_element_id).val();
			subcontractor_bid_document_file_manager_file_id = parseInputToInt(subcontractor_bid_document_file_manager_file_id);
			ajaxQueryString = ajaxQueryString + '&subcontractor_bid_document_file_manager_file_id=' + encodeURIComponent(subcontractor_bid_document_file_manager_file_id);
		}

		var subcontractor_bid_document_file_sha1_element_id = attributeGroupName + '--' + attributeSubgroupName + '--subcontractor_bid_document_file_sha1--' + uniqueId;
		if ($("#" + subcontractor_bid_document_file_sha1_element_id).length) {
			var subcontractor_bid_document_file_sha1 = $("#" + subcontractor_bid_document_file_sha1_element_id).val();
			ajaxQueryString = ajaxQueryString + '&subcontractor_bid_document_file_sha1=' + encodeURIComponent(subcontractor_bid_document_file_sha1);
		}
...
function filterSubcontractorBidDocumentHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'subcontractor_bid_document_id':
		...
		case 'subcontractor_bid_id':
			...
			inputDataFiltered = true;
			break;
		case 'subcontractor_bid_document_type_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'subcontractor_bid_document_sequence_number':
			var newValue = parseInputToMySQLUnsignedSmallInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'subcontractor_bid_document_file_manager_file_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'created':
