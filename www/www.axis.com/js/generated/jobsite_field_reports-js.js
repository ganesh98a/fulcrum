 ...
 */
function createJobsiteFieldReport(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Jobsite Field Report';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Jobsite Field Reports';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-jobsite_field_report-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'jobsite_field_reports';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'jobsite_field_reports-ajax.php?method=createJobsiteFieldReport';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildJobsiteFieldReportHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadJobsiteFieldReport(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_field_reports-ajax.php?method=loadJobsiteFieldReport';
		}
		...
		var jobsite_field_report_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_field_report_id--' + uniqueId;
		if ($("#" + jobsite_field_report_id_element_id).length) {
			var jobsite_field_report_id = $("#" + jobsite_field_report_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&jobsite_field_report_id=' + encodeURIComponent(jobsite_field_report_id);
		}
...
function loadAllJobsiteFieldReportRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_field_reports-ajax.php?method=loadAllJobsiteFieldReportRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateJobsiteFieldReport(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterJobsiteFieldReportHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_field_reports-ajax.php?method=updateJobsiteFieldReport';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllJobsiteFieldReportAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_field_reports-ajax.php?method=updateAllJobsiteFieldReportAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteJobsiteFieldReport(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Jobsite Field Report?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_field_reports-ajax.php?method=deleteJobsiteFieldReport';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveJobsiteFieldReport(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_field_reports-ajax.php?method=saveJobsiteFieldReport';
		}
...
function buildJobsiteFieldReportHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + jobsite_field_report_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					jobsite_field_report_id = parseInputToMySQLUnsignedInt(jobsite_field_report_id);
				}
				ajaxQueryString = ajaxQueryString + '&jobsite_field_report_id=' + encodeURIComponent(jobsite_field_report_id);
			}
		}
		...
		var jobsite_field_report_file_manager_file_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_field_report_file_manager_file_id--' + uniqueId;
		if ($("#" + jobsite_field_report_file_manager_file_id_element_id).length) {
			var jobsite_field_report_file_manager_file_id = $("#" + jobsite_field_report_file_manager_file_id_element_id).val();
			jobsite_field_report_file_manager_file_id = parseInputToInt(jobsite_field_report_file_manager_file_id);
			ajaxQueryString = ajaxQueryString + '&jobsite_field_report_file_manager_file_id=' + encodeURIComponent(jobsite_field_report_file_manager_file_id);
		}
...
function filterJobsiteFieldReportHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'jobsite_field_report_id':
		...
		case 'jobsite_daily_log_id':
			...
			inputDataFiltered = true;
			break;
		case 'jobsite_field_report_file_manager_file_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'sort_order':
