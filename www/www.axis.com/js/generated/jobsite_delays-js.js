 ...
 */
function createJobsiteDelay(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Jobsite Delay';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Jobsite Delays';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-jobsite_delay-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'jobsite_delays';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'jobsite_delays-ajax.php?method=createJobsiteDelay';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildJobsiteDelayHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadJobsiteDelay(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_delays-ajax.php?method=loadJobsiteDelay';
		}
		...
		var jobsite_delay_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_delay_id--' + uniqueId;
		if ($("#" + jobsite_delay_id_element_id).length) {
			var jobsite_delay_id = $("#" + jobsite_delay_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&jobsite_delay_id=' + encodeURIComponent(jobsite_delay_id);
		}
...
function loadAllJobsiteDelayRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_delays-ajax.php?method=loadAllJobsiteDelayRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateJobsiteDelay(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterJobsiteDelayHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_delays-ajax.php?method=updateJobsiteDelay';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllJobsiteDelayAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_delays-ajax.php?method=updateAllJobsiteDelayAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteJobsiteDelay(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Jobsite Delay?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_delays-ajax.php?method=deleteJobsiteDelay';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveJobsiteDelay(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_delays-ajax.php?method=saveJobsiteDelay';
		}
...
function buildJobsiteDelayHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + jobsite_delay_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					jobsite_delay_id = parseInputToMySQLUnsignedInt(jobsite_delay_id);
				}
				ajaxQueryString = ajaxQueryString + '&jobsite_delay_id=' + encodeURIComponent(jobsite_delay_id);
			}
		}
		...
		var jobsite_delay_subcategory_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_delay_subcategory_id--' + uniqueId;
		if ($("#" + jobsite_delay_subcategory_id_element_id).length) {
			var jobsite_delay_subcategory_id = $("#" + jobsite_delay_subcategory_id_element_id).val();
			jobsite_delay_subcategory_id = parseInputToInt(jobsite_delay_subcategory_id);
			ajaxQueryString = ajaxQueryString + '&jobsite_delay_subcategory_id=' + encodeURIComponent(jobsite_delay_subcategory_id);
		}

		var jobsite_building_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_building_id--' + uniqueId;
		if ($("#" + jobsite_building_id_element_id).length) {
			var jobsite_building_id = $("#" + jobsite_building_id_element_id).val();
			jobsite_building_id = parseInputToInt(jobsite_building_id);
			ajaxQueryString = ajaxQueryString + '&jobsite_building_id=' + encodeURIComponent(jobsite_building_id);
		}

		var jobsite_sitework_region_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_sitework_region_id--' + uniqueId;
		if ($("#" + jobsite_sitework_region_id_element_id).length) {
			var jobsite_sitework_region_id = $("#" + jobsite_sitework_region_id_element_id).val();
			jobsite_sitework_region_id = parseInputToInt(jobsite_sitework_region_id);
			ajaxQueryString = ajaxQueryString + '&jobsite_sitework_region_id=' + encodeURIComponent(jobsite_sitework_region_id);
		}
...
function filterJobsiteDelayHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'jobsite_delay_id':
		...
		case 'jobsite_daily_log_id':
			...
			inputDataFiltered = true;
			break;
		case 'jobsite_delay_subcategory_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'jobsite_building_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'jobsite_sitework_region_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		default:
