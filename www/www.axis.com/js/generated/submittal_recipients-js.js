 ...
 */
function createSubmittalRecipient(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Submittal Recipient';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Submittal Recipients';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-submittal_recipient-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'submittal_recipients';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'submittal_recipients-ajax.php?method=createSubmittalRecipient';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildSubmittalRecipientHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadSubmittalRecipient(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_recipients-ajax.php?method=loadSubmittalRecipient';
		}
		...
		var submittal_notification_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--submittal_notification_id--' + uniqueId;
		if ($("#" + submittal_notification_id_element_id).length) {
			var submittal_notification_id = $("#" + submittal_notification_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&submittal_notification_id=' + encodeURIComponent(submittal_notification_id);
		}
		var su_additional_recipient_contact_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--su_additional_recipient_contact_id--' + uniqueId;
		if ($("#" + su_additional_recipient_contact_id_element_id).length) {
			var su_additional_recipient_contact_id = $("#" + su_additional_recipient_contact_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&su_additional_recipient_contact_id=' + encodeURIComponent(su_additional_recipient_contact_id);
		}
...
function loadAllSubmittalRecipientRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_recipients-ajax.php?method=loadAllSubmittalRecipientRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateSubmittalRecipient(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterSubmittalRecipientHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_recipients-ajax.php?method=updateSubmittalRecipient';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllSubmittalRecipientAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_recipients-ajax.php?method=updateAllSubmittalRecipientAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteSubmittalRecipient(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Submittal Recipient?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_recipients-ajax.php?method=deleteSubmittalRecipient';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveSubmittalRecipient(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_recipients-ajax.php?method=saveSubmittalRecipient';
		}
...
function buildSubmittalRecipientHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if ($("#" + submittal_notification_id_element_id).length) {
			var submittal_notification_id = $("#" + submittal_notification_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					submittal_notification_id = parseInputToMySQLUnsignedInt(submittal_notification_id);
				}
			ajaxQueryString = ajaxQueryString + '&submittal_notification_id=' + encodeURIComponent(submittal_notification_id);
		}
		...
		if ($("#" + su_additional_recipient_contact_id_element_id).length) {
			var su_additional_recipient_contact_id = $("#" + su_additional_recipient_contact_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					su_additional_recipient_contact_id = parseInputToMySQLUnsignedInt(su_additional_recipient_contact_id);
				}
			ajaxQueryString = ajaxQueryString + '&su_additional_recipient_contact_id=' + encodeURIComponent(su_additional_recipient_contact_id);
		}

		var su_additional_recipient_contact_mobile_phone_number_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--su_additional_recipient_contact_mobile_phone_number_id--' + uniqueId;
		if ($("#" + su_additional_recipient_contact_mobile_phone_number_id_element_id).length) {
			var su_additional_recipient_contact_mobile_phone_number_id = $("#" + su_additional_recipient_contact_mobile_phone_number_id_element_id).val();
			su_additional_recipient_contact_mobile_phone_number_id = parseInputToInt(su_additional_recipient_contact_mobile_phone_number_id);
			ajaxQueryString = ajaxQueryString + '&su_additional_recipient_contact_mobile_phone_number_id=' + encodeURIComponent(su_additional_recipient_contact_mobile_phone_number_id);
		}
...
function filterSubmittalRecipientHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'submittal_notification_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'su_additional_recipient_contact_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'su_additional_recipient_contact_mobile_phone_number_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		default:
