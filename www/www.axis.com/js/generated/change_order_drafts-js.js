 ...
 */
function createChangeOrderDraft(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Change Order Draft';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Change Order Drafts';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-change_order_draft-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'change_order_drafts';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'change_order_drafts-ajax.php?method=createChangeOrderDraft';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildChangeOrderDraftHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadChangeOrderDraft(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_drafts-ajax.php?method=loadChangeOrderDraft';
		}
		...
		var change_order_draft_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--change_order_draft_id--' + uniqueId;
		if ($("#" + change_order_draft_id_element_id).length) {
			var change_order_draft_id = $("#" + change_order_draft_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&change_order_draft_id=' + encodeURIComponent(change_order_draft_id);
		}
...
function loadAllChangeOrderDraftRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_drafts-ajax.php?method=loadAllChangeOrderDraftRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateChangeOrderDraft(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterChangeOrderDraftHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_drafts-ajax.php?method=updateChangeOrderDraft';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllChangeOrderDraftAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_drafts-ajax.php?method=updateAllChangeOrderDraftAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteChangeOrderDraft(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Change Order Draft?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_drafts-ajax.php?method=deleteChangeOrderDraft';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveChangeOrderDraft(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_drafts-ajax.php?method=saveChangeOrderDraft';
		}
...
function buildChangeOrderDraftHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + change_order_draft_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					change_order_draft_id = parseInputToMySQLUnsignedInt(change_order_draft_id);
				}
				ajaxQueryString = ajaxQueryString + '&change_order_draft_id=' + encodeURIComponent(change_order_draft_id);
			}
		}
...
function filterChangeOrderDraftHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'change_order_draft_id':
