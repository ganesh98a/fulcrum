 ...
 */
function createRequestForInformationRecipient(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Request For Information Recipient';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Request For Information Recipients';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-request_for_information_recipient-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'request_for_information_recipients';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'request_for_information_recipients-ajax.php?method=createRequestForInformationRecipient';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildRequestForInformationRecipientHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadRequestForInformationRecipient(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'request_for_information_recipients-ajax.php?method=loadRequestForInformationRecipient';
		}
		...
		var request_for_information_notification_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--request_for_information_notification_id--' + uniqueId;
		if ($("#" + request_for_information_notification_id_element_id).length) {
			var request_for_information_notification_id = $("#" + request_for_information_notification_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&request_for_information_notification_id=' + encodeURIComponent(request_for_information_notification_id);
		}
		var rfi_additional_recipient_contact_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--rfi_additional_recipient_contact_id--' + uniqueId;
		if ($("#" + rfi_additional_recipient_contact_id_element_id).length) {
			var rfi_additional_recipient_contact_id = $("#" + rfi_additional_recipient_contact_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&rfi_additional_recipient_contact_id=' + encodeURIComponent(rfi_additional_recipient_contact_id);
		}
...
function loadAllRequestForInformationRecipientRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'request_for_information_recipients-ajax.php?method=loadAllRequestForInformationRecipientRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateRequestForInformationRecipient(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterRequestForInformationRecipientHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'request_for_information_recipients-ajax.php?method=updateRequestForInformationRecipient';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllRequestForInformationRecipientAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'request_for_information_recipients-ajax.php?method=updateAllRequestForInformationRecipientAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteRequestForInformationRecipient(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Request For Information Recipient?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'request_for_information_recipients-ajax.php?method=deleteRequestForInformationRecipient';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveRequestForInformationRecipient(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'request_for_information_recipients-ajax.php?method=saveRequestForInformationRecipient';
		}
...
function buildRequestForInformationRecipientHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if ($("#" + request_for_information_notification_id_element_id).length) {
			var request_for_information_notification_id = $("#" + request_for_information_notification_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					request_for_information_notification_id = parseInputToMySQLUnsignedInt(request_for_information_notification_id);
				}
			ajaxQueryString = ajaxQueryString + '&request_for_information_notification_id=' + encodeURIComponent(request_for_information_notification_id);
		}
		...
		if ($("#" + rfi_additional_recipient_contact_id_element_id).length) {
			var rfi_additional_recipient_contact_id = $("#" + rfi_additional_recipient_contact_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					rfi_additional_recipient_contact_id = parseInputToMySQLUnsignedInt(rfi_additional_recipient_contact_id);
				}
			ajaxQueryString = ajaxQueryString + '&rfi_additional_recipient_contact_id=' + encodeURIComponent(rfi_additional_recipient_contact_id);
		}

		var rfi_additional_recipient_contact_mobile_phone_number_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--rfi_additional_recipient_contact_mobile_phone_number_id--' + uniqueId;
		if ($("#" + rfi_additional_recipient_contact_mobile_phone_number_id_element_id).length) {
			var rfi_additional_recipient_contact_mobile_phone_number_id = $("#" + rfi_additional_recipient_contact_mobile_phone_number_id_element_id).val();
			rfi_additional_recipient_contact_mobile_phone_number_id = parseInputToInt(rfi_additional_recipient_contact_mobile_phone_number_id);
			ajaxQueryString = ajaxQueryString + '&rfi_additional_recipient_contact_mobile_phone_number_id=' + encodeURIComponent(rfi_additional_recipient_contact_mobile_phone_number_id);
		}

		var smtp_recipient_header_type_element_id = attributeGroupName + '--' + attributeSubgroupName + '--smtp_recipient_header_type--' + uniqueId;
		if ($("#" + smtp_recipient_header_type_element_id).length) {
			var smtp_recipient_header_type = $("#" + smtp_recipient_header_type_element_id).val();
			ajaxQueryString = ajaxQueryString + '&smtp_recipient_header_type=' + encodeURIComponent(smtp_recipient_header_type);
		}
...
function filterRequestForInformationRecipientHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'request_for_information_notification_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'rfi_additional_recipient_contact_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'rfi_additional_recipient_contact_mobile_phone_number_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		default:
