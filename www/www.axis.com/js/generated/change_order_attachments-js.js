 ...
 */
function createChangeOrderAttachment(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Change Order Attachment';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Change Order Attachments';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-change_order_attachment-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'change_order_attachments';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'change_order_attachments-ajax.php?method=createChangeOrderAttachment';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildChangeOrderAttachmentHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadChangeOrderAttachment(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_attachments-ajax.php?method=loadChangeOrderAttachment';
		}
		...
		var co_attachment_file_manager_file_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--co_attachment_file_manager_file_id--' + uniqueId;
		if ($("#" + co_attachment_file_manager_file_id_element_id).length) {
			var co_attachment_file_manager_file_id = $("#" + co_attachment_file_manager_file_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&co_attachment_file_manager_file_id=' + encodeURIComponent(co_attachment_file_manager_file_id);
		}
...
function loadAllChangeOrderAttachmentRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_attachments-ajax.php?method=loadAllChangeOrderAttachmentRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateChangeOrderAttachment(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterChangeOrderAttachmentHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_attachments-ajax.php?method=updateChangeOrderAttachment';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllChangeOrderAttachmentAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_attachments-ajax.php?method=updateAllChangeOrderAttachmentAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteChangeOrderAttachment(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Change Order Attachment?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_attachments-ajax.php?method=deleteChangeOrderAttachment';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveChangeOrderAttachment(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'change_order_attachments-ajax.php?method=saveChangeOrderAttachment';
		}
...
function buildChangeOrderAttachmentHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if ($("#" + co_attachment_file_manager_file_id_element_id).length) {
			var co_attachment_file_manager_file_id = $("#" + co_attachment_file_manager_file_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					co_attachment_file_manager_file_id = parseInputToMySQLUnsignedInt(co_attachment_file_manager_file_id);
				}
			ajaxQueryString = ajaxQueryString + '&co_attachment_file_manager_file_id=' + encodeURIComponent(co_attachment_file_manager_file_id);
		}

		var co_attachment_source_contact_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--co_attachment_source_contact_id--' + uniqueId;
		if ($("#" + co_attachment_source_contact_id_element_id).length) {
			var co_attachment_source_contact_id = $("#" + co_attachment_source_contact_id_element_id).val();
			co_attachment_source_contact_id = parseInputToInt(co_attachment_source_contact_id);
			ajaxQueryString = ajaxQueryString + '&co_attachment_source_contact_id=' + encodeURIComponent(co_attachment_source_contact_id);
		}
...
function filterChangeOrderAttachmentHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	...
	switch (attributeName) {
		case 'change_order_id':
			...
			inputDataFiltered = true;
			break;
		case 'co_attachment_file_manager_file_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'co_attachment_source_contact_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'sort_order':
