 ...
 */
function createJobsitePhoto(attributeGroupName, uniqueId, options,inter)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			if(inter=='Y'){
				htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-jobsite_internal_photo-record';
			}else
			{
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			if(inter=='Y'){
				var attributeSubgroupName = 'jobsite_internal_photo';
			}else
			{
		...
		var ajaxQueryString =
			...
			'&internal_use_only_flag=' + encodeURIComponent(inter);
...
function loadJobsitePhoto(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_photos-ajax.php?method=loadJobsitePhoto';
		}
		...
		var jobsite_photo_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_photo_id--' + uniqueId;
		if ($("#" + jobsite_photo_id_element_id).length) {
			var jobsite_photo_id = $("#" + jobsite_photo_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&jobsite_photo_id=' + encodeURIComponent(jobsite_photo_id);
		}
...
function loadAllJobsitePhotoRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_photos-ajax.php?method=loadAllJobsitePhotoRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateJobsitePhoto(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterJobsitePhotoHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_photos-ajax.php?method=updateJobsitePhoto';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllJobsitePhotoAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_photos-ajax.php?method=updateAllJobsitePhotoAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteJobsitePhoto(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Jobsite Photo?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_photos-ajax.php?method=deleteJobsitePhoto';
		}
...
// gallery view for the DCR images
function galleryView(jobsite_daily_log_id)
{

	var ajaxHandlerScript = 'jobsite_photos-ajax.php?method=galleryviewforDCR';
	var ajaxQueryString =
	...
	var returnedJqXHR = $.ajax({
		...
		success: function(res)
		{
			...
			$("#modalDialogContainer").html(res);
			$("#modalDialogContainer").removeClass('hidden');
			$("#modalDialogContainer").dialog({
				modal: true,
				title: 'DCR — '+$("#currentlySelectedUserCompanyName").val()+' — '+$("#currentlySelectedProjectName").val(),
				width: dialogWidth,
				height: dialogHeight,
				open: function() {
				...
				buttons: {
					...
					'Close': function() {
						$("#modalDialogContainer").dialog('close');
					}
				}
			});
		},
...
// To Add/update the caption for the image
function updatecaptionforimage(element,fileid,jobsitephotoid)
{
	
	var captionval = $("#"+element).val();
	var ajaxHandlerScript = 'jobsite_photos-ajax.php?method=updatecaptionforimage';
	var ajaxQueryString =
	'&fileid=' + fileid+
	'&captionval='+encodeURIComponent(captionval)+
	'&jobsitephotoid='+jobsitephotoid;
	...
	var returnedJqXHR = $.ajax({
		...
		success: function(res)
		{
			if(res=='1')
			{
				messageAlert('Successfully saved the Caption', 'successMessage');

			}else
			{
				messageAlert('Not updated', 'errorMessage');
			}
		},
		});
 ...
 * options is an object with a collection of optional directives
 */
function saveJobsitePhoto(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'jobsite_photos-ajax.php?method=saveJobsitePhoto';
		}
...
function buildJobsitePhotoHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (!optionsObjectIsEmpty && options.includeId) {
			...
			if ($("#" + jobsite_photo_id_element_id).length) {
				...
				if (!optionsObjectIsEmpty && options.filterId) {
					jobsite_photo_id = parseInputToMySQLUnsignedInt(jobsite_photo_id);
				}
				ajaxQueryString = ajaxQueryString + '&jobsite_photo_id=' + encodeURIComponent(jobsite_photo_id);
			}
		}
		...
		var jobsite_photo_file_manager_file_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--jobsite_photo_file_manager_file_id--' + uniqueId;
		if ($("#" + jobsite_photo_file_manager_file_id_element_id).length) {
			var jobsite_photo_file_manager_file_id = $("#" + jobsite_photo_file_manager_file_id_element_id).val();
			jobsite_photo_file_manager_file_id = parseInputToInt(jobsite_photo_file_manager_file_id);
			ajaxQueryString = ajaxQueryString + '&jobsite_photo_file_manager_file_id=' + encodeURIComponent(jobsite_photo_file_manager_file_id);
		}

		var internal_use_only_flag_element_id = attributeGroupName + '--' + attributeSubgroupName + '--internal_use_only_flag--' + uniqueId;
		if ($("#" + internal_use_only_flag_element_id).length) {
			var internal_use_only_flag = $("#" + internal_use_only_flag_element_id).val();
			if ($("#" + internal_use_only_flag_element_id).is(':checkbox')) {
				if ($("#" + internal_use_only_flag_element_id).is(':checked')) {
					internal_use_only_flag = 'Y';
				} else {
					internal_use_only_flag = 'N';
				}
			}
			ajaxQueryString = ajaxQueryString + '&internal_use_only_flag=' + encodeURIComponent(internal_use_only_flag);
		}
...
function filterJobsitePhotoHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	inputDataFiltered = false;

	switch (attributeName) {
		case 'jobsite_photo_id':
		...
		case 'jobsite_daily_log_id':
			...
			inputDataFiltered = true;
			break;
		case 'jobsite_photo_file_manager_file_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'sort_order':
