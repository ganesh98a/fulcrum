 ...
 */
function createSubmittalAttachment(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.htmlRecordMetaAttributes) {
			...
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeGroupName		= 'Submittal Attachment';
			htmlRecordMetaAttributesOptions.defaultFormattedAttributeSubgroupName	= 'Submittal Attachments';
			htmlRecordMetaAttributesOptions.defaultNewAttributeGroupName			= 'manage-submittal_attachment-record';
		...
		if (!optionsObjectIsEmpty && options.attributeSubgroupName) {
			...
			var attributeSubgroupName = 'submittal_attachments';
		}

		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			var ajaxHandlerScript = options.ajaxHandlerScript;
		} else {
			var ajaxHandlerScript = 'submittal_attachments-ajax.php?method=createSubmittalAttachment';
		}
		...
		if (!optionsObjectIsEmpty && options.skipBuildHtmlRecordAttributesAsAjaxQueryString) {
			...
			var htmlRecordAttributesAsAjaxQueryString = buildSubmittalAttachmentHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, htmlRecordAttributeOptions);
...
function loadSubmittalAttachment(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_attachments-ajax.php?method=loadSubmittalAttachment';
		}
		...
		var su_attachment_file_manager_file_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--su_attachment_file_manager_file_id--' + uniqueId;
		if ($("#" + su_attachment_file_manager_file_id_element_id).length) {
			var su_attachment_file_manager_file_id = $("#" + su_attachment_file_manager_file_id_element_id).val();
			ajaxQueryString = ajaxQueryString + '&su_attachment_file_manager_file_id=' + encodeURIComponent(su_attachment_file_manager_file_id);
		}
...
function loadAllSubmittalAttachmentRecords(recordListContainerElementId, attributeGroupName, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_attachments-ajax.php?method=loadAllSubmittalAttachmentRecords';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateSubmittalAttachment(element, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!inputDataFiltered) {
			var objReturn = filterSubmittalAttachmentHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue);
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_attachments-ajax.php?method=updateSubmittalAttachment';
		}
 ...
 * options is an object with a collection of optional directives
 */
function updateAllSubmittalAttachmentAttributes(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_attachments-ajax.php?method=updateAllSubmittalAttachmentAttributes';
		}
 ...
 * options is an object with a collection of optional directives
 */
function deleteSubmittalAttachment(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (confirmDelete) {
			var confirmDeleteAnswer = window.confirm('Are You Sure That You Want To Delete This Submittal Attachment?');
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_attachments-ajax.php?method=deleteSubmittalAttachment';
		}
...
function deleteSubmittalAttachmentDraft(recordContainerElementId, attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_attachments-ajax.php?method=deleteSubmittalAttachmentDraft';
		}
 ...
 * options is an object with a collection of optional directives
 */
function saveSubmittalAttachment(attributeGroupName, uniqueId, options)
{
	// Debug
	//return;
	...
	try {
		...
		if (!optionsObjectIsEmpty && options.ajaxHandlerScript) {
			...
			var ajaxHandlerScript = 'submittal_attachments-ajax.php?method=saveSubmittalAttachment';
		}
...
function buildSubmittalAttachmentHtmlRecordAttributesAsAjaxQueryString(attributeGroupName, uniqueId, options)
{
	try {
		...
		if ($("#" + su_attachment_file_manager_file_id_element_id).length) {
			var su_attachment_file_manager_file_id = $("#" + su_attachment_file_manager_file_id_element_id).val();
				if (!optionsObjectIsEmpty && options.filterId) {
					su_attachment_file_manager_file_id = parseInputToMySQLUnsignedInt(su_attachment_file_manager_file_id);
				}
			ajaxQueryString = ajaxQueryString + '&su_attachment_file_manager_file_id=' + encodeURIComponent(su_attachment_file_manager_file_id);
		}

		var su_attachment_source_contact_id_element_id = attributeGroupName + '--' + attributeSubgroupName + '--su_attachment_source_contact_id--' + uniqueId;
		if ($("#" + su_attachment_source_contact_id_element_id).length) {
			var su_attachment_source_contact_id = $("#" + su_attachment_source_contact_id_element_id).val();
			su_attachment_source_contact_id = parseInputToInt(su_attachment_source_contact_id);
			ajaxQueryString = ajaxQueryString + '&su_attachment_source_contact_id=' + encodeURIComponent(su_attachment_source_contact_id);
		}
...
function filterSubmittalAttachmentHtmlRecordAttributeValueByAttributeName(attributeName, tmpValue)
{
	...
	switch (attributeName) {
		case 'submittal_id':
			...
			inputDataFiltered = true;
			break;
		case 'su_attachment_file_manager_file_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'su_attachment_source_contact_id':
			var newValue = parseInputToMySQLUnsignedInt(tmpValue);
			inputDataFiltered = true;
			break;
		case 'sort_order':
