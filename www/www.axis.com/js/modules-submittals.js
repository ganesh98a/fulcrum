...
$(document).ready(function() {
	...
	if(search!='')
	{
		var id = search.split('=');
		var sub_type=id[1];
		var sub_id=sub_type.split('draft_');
		if(sub_id[1]>0 || sub_id[1]!=''){
			sRegisryToS(sub_id[1]);
...
function sRegisryToS(id)
{
	try {
		...
		var ajaxHandlerScript = 'submittal_drafts-ajax.php?method=createSubmittalDraftSR';

		var ajaxHandler = window.ajaxUrlPrefix + ajaxHandlerScript;
		var ajaxQueryString = 'id=' + id;
		...
		if (skipDefaultSuccessCallback) {
			...
			var arrSuccessCallbacks = [ Submittals__loadDraftDialogSuccess ];
		}
...
function Submittals__loadDraftDialogSuccess(data, textStatus, jqXHR)
{
	try {

		var json = data;
		var id = json.id;
		Submittals__loadCreateSuDialog('','',id);
	} catch(error) {
...
function Submittals__loadCreateSuDialog(element, options,sDrafId='')
{
	try {
		...
		var ajaxHandler = window.ajaxUrlPrefix + 'modules-submittals-ajax.php?method=Submittals__loadCreateSuDialog';
		var ajaxQueryString =
...
function Submittals__loadCreateSuDialogSuccess(data, textStatus, jqXHR)
{
	try {
		...
		if (errorNumber == 0) {
			...
			$("#divCreateSu").html(htmlContent);
			$("#divCreateSu").removeClass('hidden');
			$("#divCreateSu").dialog({
				modal: true,
				title: 'Create A New Submittal — '+$("#currentlySelectedUserCompanyName").val()+' — '+$("#currentlySelectedProjectName").val(),
				width: dialogWidth,
				height: dialogHeight,
				open: function() {
				...
				close: function() {
					...
					$("#divCreateSu").addClass('hidden');
					$("#divCreateSu").dialog('destroy');
					$("#divCreateSu").empty().html('');
				},
				buttons: {
					'Close': function() {
...
function Submittals__createSuViaPromiseChain(attributeGroupName, uniqueId)
{
	//To Hide the submit button once it is trigged to restrict double entries
	$("#SuResponseAndSendEmailViaPromiseChain").attr('disabled',true);
	$("#SuResponseViaPromiseChain").attr('disabled',true);
...
function Submittals__deleteSubmittalDraftSuccess(data, textStatus, jqXHR)
{
	try {
		...
		if (errorNumber == 0) {
			...
			var uniqueId = json.uniqueId;
			//Submittals__loadCreateSuDialog();
...
function Submittals__createSubmittal(attributeGroupName, uniqueId, options)
{
	try {
		...
		if (isEmail == 0 && (isNotes == '' || isTitle == '') ) {
			$("#ddl--create-submittal-record--submittals--su_recipient_contact_id--"+uniqueId).parent().children(':first-child').addClass('redBorderThick');
			var promise = createSubmittal(attributeGroupName, uniqueId, options);
			$isErr = 1;
		}else if (isEmail == 0) {
			$("#ddl--create-submittal-record--submittals--su_recipient_contact_id--"+uniqueId).parent().children(':first-child').addClass('redBorderThick');
