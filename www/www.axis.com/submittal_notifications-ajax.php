...
if (isset($_SERVER['HTTP_HOST']) && isset($_SERVER['PHP_SELF'])) {
	if (($_SERVER['HTTP_HOST'] == 'localdev.example.com') && is_int(strpos($_SERVER['PHP_SELF'], '/auto/'))) {
		require_once('submittal_notifications-functions.php');
	}
}
...
$userCanViewSubmittalNotifications = $permissions->determineAccessToSoftwareModuleFunction('submittal_notifications_view');
$userCanManageSubmittalNotifications = $permissions->determineAccessToSoftwareModuleFunction('submittal_notifications_manage');
*/
...
switch ($methodCall) {
	case 'createSubmittalNotification':
		...
		try {
			...
			if ($enforcePermissions) {
				...
				$arrRequiredOrPermissions = array(
					'submittal_notifications_manage' => 1
				);
				//$errorMessage = 'Permission denied.';
				//$errorMessage = 'Permission denied: cannot create new Submittal Notification data values.';
				$arrErrorMessages = array(
					'Error creating: Submittal Notification.<br>Permission Denied.' => 1
				);
			...
			if (!isset($formattedAttributeGroupName) || empty($formattedAttributeGroupName)) {
				$formattedAttributeGroupName = 'Submittal Notification';
			}
			if (!isset($formattedAttributeSubgroupName) || empty($formattedAttributeSubgroupName)) {
				$formattedAttributeSubgroupName = 'Submittal Notifications';
			}
			if (!isset($newAttributeGroupName) || empty($newAttributeGroupName)) {
				$newAttributeGroupName = 'manage-submittal_notification-record';
			}
			...
			$submittal_notification_timestamp = (string) $get->submittal_notification_timestamp;
			*/
			...
			// Retrieve all of the $_GET inputs automatically for the SubmittalNotification record
			...
			$submittal_id = intVal($httpGetInputData['submittal_id']);
			$submittal_notification_id = SubmittalNotification::findSubmittalNotificationBySubmittalIdExtended($database, $submittal_id);

			$submittalNotification = new SubmittalNotification($database);

			if($submittal_notification_id ==  false ){
			
			$submittalNotification->setData($httpGetInputData);
			$submittalNotification->convertDataToProperties();

			/*
			$submittalNotification->submittal_notification_id = $submittal_notification_id;
			$submittalNotification->submittal_id = $submittal_id;
			$submittalNotification->submittal_notification_timestamp = $submittal_notification_timestamp;
			*/

			$submittalNotification->convertPropertiesToData();
			$data = $submittalNotification->getData();

			// Test for existence via standard findByUniqueIndex method
			$submittalNotification->findByUniqueIndex();
			if ($submittalNotification->isDataLoaded()) {
				// Error code here
				$errorMessage = 'Submittal Notification already exists.';
				$message->enqueueError($errorMessage, $currentPhpScript);
				$primaryKeyAsString = $submittalNotification->getPrimaryKeyAsString();
				throw new Exception($errorMessage);
				//$error->outputErrorMessages();
				//exit;
			} else {
				$submittalNotification->setKey(null);
				$submittalNotification->setData($data);
			}

			$submittal_notification_id = $submittalNotification->save();
			}
			if (isset($submittal_notification_id) && !empty($submittal_notification_id)) {
				$submittalNotification->submittal_notification_id = $submittal_notification_id;
				$submittalNotification->setId($submittal_notification_id);
			}
			// $submittalNotification->save();
			...
			if (isset($primaryKeyAsString) && (!empty($primaryKeyAsString))) {
				...
				// Error code here
				$errorNumber = 1;
				$errorMessage = 'Error creating: Submittal Notification.';
				$message->enqueueError($errorMessage, $currentPhpScript);
				throw new Exception($errorMessage);
				//$error->outputErrorMessages();
				//exit;
		...
		if (!isset($primaryKeyAsString)) {
			$primaryKeyAsString = '';
			if (isset($submittalNotification) && $submittalNotification instanceof SubmittalNotification) {
				$primaryKeyAsString = $submittalNotification->getPrimaryKeyAsString();
			}
		}
	...
	case 'loadSubmittalNotification':
		...
		try {
			...
			if ($enforcePermissions) {
				$arrRequiredAndPermissions = array();
				$arrRequiredOrPermissions = array(
					'submittal_notifications_view' => 1
				);
				//$errorMessage = 'Permission denied.';
				$arrErrorMessages = array(
					'Error loading: Submittal Notification.<br>Permission Denied.' => 1
				);
				require('code-generator/ajax-permissions.php');
			}
			...
			//$primaryKeyAsString = '';
			//$errorMessage = 'Error loading: Submittal Notification';
			//$message->enqueueError($errorMessage, $currentPhpScript);
			//$backTrace = getExceptionTraceAsString($e);
			//$error->setBackTrace($backTrace);
			//$error->outputErrorMessages();
	...
	case 'loadAllSubmittalNotificationRecords':
		...
		try {
			...
			//echo "$errorNumber|$errorMessage|submittal_notifications|Submittal Notification|$htmlContent";
	...
	case 'updateSubmittalNotification':
		...
		try {
			...
			$message->reset($currentPhpScript);
			$errorMessage = 'Error updating: Submittal Notification';
			$message->enqueueError($errorMessage, $currentPhpScript);
			...
			if ($enforcePermissions) {
				...
				//$errorMessage = 'Permission denied.';
				$arrErrorMessages = array(
					"Error updating Submittal Notification - {$attributeName}.<br>Permission Denied." => 1
				);
				require('code-generator/ajax-permissions.php');
			}
			...
			$arrAllowableAttributes = array(
				...
				'submittal_notification_timestamp' => 'submittal notification timestamp',
			);
			...
			if ($attributeSubgroupName == 'submittal_notifications') {
				// @todo Add in $previousId logic to check if a candidate key attribute value changed
				$submittal_notification_id = (int) $uniqueId;

				// Put in findById() or findByUniqueKey() as appropriate
				$submittalNotification = SubmittalNotification::findById($database, $submittal_notification_id);
				/* @var $submittalNotification SubmittalNotification */

				if ($submittalNotification) {
					// Check if the value actually changed
					$existingValue = $submittalNotification->$attributeName;
					...
					$arrAjaxUniqueIndexAttributes = array(
						'submittal_id' => 1,
						'submittal_notification_timestamp' => 1,
					);
					if (isset($arrAjaxUniqueIndexAttributes[$attributeName])) {
						$existingValue = $submittalNotification->$attributeName;
						$submittalNotification->$attributeName = $newValue;
						$possibleDuplicateSubmittalNotification = SubmittalNotification::findBySubmittalIdAndSubmittalNotificationTimestamp($database, $submittalNotification->submittal_id, $submittalNotification->submittal_notification_timestamp);
						if ($possibleDuplicateSubmittalNotification) {
							$save = false;
							$resetToPreviousValue = 'Y';
							$errorMessage = "Submittal Notification $newValue already exists.";

							//$message->enqueueError($errorMessage, $currentPhpScript);
							//$error->outputErrorMessages();
							//exit;
						} else {
							$submittalNotification->$attributeName = $existingValue;
						}
					}
					...
					$errorNumber = 1;
					$errorMessage = 'Submittal Notification record does not exist.';
					$message->enqueueError($errorMessage, $currentPhpScript);
					throw new Exception($errorMessage);
					//$error->outputErrorMessages();
					//exit;
	...
	case 'updateAllSubmittalNotificationAttributes':
		...
		try {
			...
			if ($enforcePermissions) {
				...
				//$errorMessage = 'Permission denied.';
				$arrErrorMessages = array(
					'Error updating Submittal Notification.<br>Permission Denied.' => 1
				);
				require('code-generator/ajax-permissions.php');
			}
			...
			*/
			...
			if ($attributeSubgroupName == 'submittal_notifications') {
				...
				if ($submittalNotification) {
					$existingData = $submittalNotification->getData();
					...
					*/
					...
					$newData = $submittalNotification->getData();
					...
					if (!empty($data)) {
						...
						$errorNumber = 1;
						$errorMessage = 'Error updating: Submittal Notification<br>No Changes In Values';
						$message->enqueueError($errorMessage, $currentPhpScript);
						$primaryKeyAsString = $submittalNotification->getPrimaryKeyAsString();
						throw new Exception($errorMessage);
					}
	...
	case 'deleteSubmittalNotification':
		...
		try {
			...
			if ($enforcePermissions) {
				...
				//$errorMessage = 'Permission denied.';
				$arrErrorMessages = array(
					'Error deleting Submittal Notification.<br>Permission Denied.' => 1
				);
				require('code-generator/ajax-permissions.php');
			}
			...
			//$primaryKeyAsString = '';
			//$errorMessage = 'Error deleting: Submittal Notification';
			//$message->enqueueError($errorMessage, $currentPhpScript);
			//$backTrace = getExceptionTraceAsString($e);
			//$error->setBackTrace($backTrace);
			//$error->outputErrorMessages();
	...
	case 'saveSubmittalNotification':
		...
		try {
			...
			if ($enforcePermissions) {
				...
				//$errorMessage = 'Permission denied.';
				//$errorMessage = 'Permission denied: cannot save new Submittal Notification data values.';
				$arrErrorMessages = array(
					'Error saving Submittal Notification.<br>Permission Denied.' => 1
				);
				require('code-generator/ajax-permissions.php');
			}
			...
			*/
			...
			$submittal_notification_id = $submittalNotification->insertOnDuplicateKeyUpdate();
			if (isset($submittal_notification_id) && !empty($submittal_notification_id)) {
				$submittalNotification->submittal_notification_id = $submittal_notification_id;
				$submittalNotification->setId($submittal_notification_id);
			}
			// $submittalNotification->insertOnDuplicateKeyUpdate();
			// $submittalNotification->insertIgnore();
