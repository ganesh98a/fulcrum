...
$RN_method_save = $RN_params['method_save'];
...
if($RN_id == null && $RN_id == ''){
	$RN_jsonEC['status'] = 400;
	$RN_jsonEC['err_message'] = "Id is Required";
}else
if($RN_method_save == null && $RN_method_save == ''){
	$RN_jsonEC['status'] = 400;
	$RN_jsonEC['err_message'] = "Method Save is Required";
}else
if($RN_method_save == null && $RN_method_save == ''){
	$RN_jsonEC['status'] = 400;
	$RN_jsonEC['err_message'] = "Method Save is Required";
...
$RN_jsonEC['data']['raw_file_data_images_view'] = null;
...
if ($RN_debugMode) {
	...
	if ($RN_uploadTempFilesOnly && !empty($RN_arrFiles)) {
		...
		$RN_jsonEC['data']['file_data'][0]['jobsite_daily_log_id'] = $RN_jobsite_daily_log_id;
	...
	if (!empty($RN_arrFiles)) {
		...
		foreach ($RN_arrFiles as $RN_file) {
			...
			$RN_post_upload_js_callback = $RN_get->post_upload_js_callback;
			...
			$RN_arrFilesMetaData[0]['file_access'] = $accessFiles;
			$RN_arrFilesMetaDataTemp['file_access'] = $accessFiles;
			if($RN_explode[0] == 'image') {
				$RN_jsonEC['data']['raw_file_data_images_view']['url'] = $RN_fileUrl.$RN_fileid;
			}
		}
	} else {
	...
	$RN_jsonEC['data']['file_data'][0]['jobsite_daily_log_id'] = $RN_jobsite_daily_log_id;
	$RN_jsonEC['err_message'] = null;
	include_once('UploadDocSave.php');
	header('X-PHP-Response-Code: '.$RN_jsonEC['status'], true, $RN_jsonEC['status']);
	$RN_ajaxMessage = json_encode($RN_jsonEC);
	echo $RN_ajaxMessage;
	exit(0);
