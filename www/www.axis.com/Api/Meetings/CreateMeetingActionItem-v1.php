...
try {
	...
	*/
	...
	if(isset($action_item_id) && $action_item_id !== null) {
	...
	if ($actionItem->isDataLoaded()) {
		// Error code here
		$RN_errorMessage = 'Action Item already exists.';
	...
	if (isset($primaryKeyAsString) && (!empty($primaryKeyAsString))) {
		...
		// Error code here
		$errorNumber = 1;
		$RN_errorMessage = 'Error creating: Action Item.';
		$RN_jsonEC['status'] = 400;
		$RN_jsonEC['data'] = null;
		header('X-PHP-Response-Code: '.$RN_jsonEC['status'], true, $RN_jsonEC['status']);
		$RN_jsonEC['err_message'] = $RN_errorMessage;
	...
	$RN_jsonEC['data']['meetingData'] = $datas;
