...
$objPHPExcel->setActiveSheetIndex()->mergeCells('F'.$index.':H'.$index);
...
/*cell background color*/
$objPHPExcel->getActiveSheet()->getStyle('A'.$index.':H'.$index)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('2481c3');
$index = $index + 2;//Fetch the Data's
$alphas = range('A', 'Z');
$alphaCharIn = 0;
...
$objPHPExcel->getActiveSheet()->getStyle('A'.$index.':H'.$index)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('2481c3');
/*cell font color*/
$objPHPExcel->getActiveSheet()->getStyle('A'.$index.':H'.$index)->getFont()->getColor()->setRGB('FFFF');
$index++;
...
$arrSubmittals = Submittal::loadSubmittalsByProjectIdAndDate($database, $RN_project_id, $loadSubmittalsByProjectIdOptions,$RN_reportsStartDate,$RN_reportsEndDate);
...
foreach ($arrSubmittals as $submittal_id => $submittal) {
    ...
    if ($suCostCode) {
        ...
        $formattedSuCostCode = $suCostCode->getFormattedCostCodeApi($database, false, $RN_user_company_id);
    ...
    foreach ($record_notes as $key => $notes) {
        $NotesValue.=$ivalue.')'.$notes['submittal_response']."\n";
        $resdate = DateTime::createFromFormat('Y-m-d H:i:s', $notes['created']);
        $Respdate = $resdate->format('m/d/Y g:i a');
        $responseDate.=$ivalue.')'.$Respdate."\n";
        $ivalue++;
    }
    $NotesValue=rtrim($NotesValue,"\n");
    ...
    $alphaCharIn = 1;
    $index++;
    $objPHPExcel->setActiveSheetIndex()->mergeCells($alphas[$alphaCharIn].$index.':'.$alphas[($alphaCharIn+5)].$index);
    ...
    $alphaCharIn = $alphaCharIn+6;
        // $objPHPExcel->setActiveSheetIndex()->mergeCells($alphas[$alphaCharIn].$index.':'.$alphas[($alphaCharIn+)].$index);
    $objPHPExcel->getActiveSheet()->setCellValue($alphas[$alphaCharIn].$index,"Response Date :");
    $objPHPExcel->getActiveSheet()->getStyle($alphas[$alphaCharIn].$index)->getFont()->setBold(true);
    $index++;
    $alphaCharIn=1;
    $objPHPExcel->setActiveSheetIndex()->mergeCells($alphas[$alphaCharIn].$index.':'.$alphas[($alphaCharIn+5)].$index);
    $NotesValue = mb_convert_encoding($NotesValue, "HTML-ENTITIES", "UTF-8");
    $objPHPExcel->getActiveSheet()->setCellValue($alphas[$alphaCharIn].$index,$NotesValue);
