...
include_once('./Reports/GeneratePdfData.php');
	...
	$groupdivision_completed = BidList::findBidlistByCompanyProjectId($database, $RN_project_id, $RN_user_company_id);
	...
	// Cost per SF/Unit
	$project = Project::findProjectByIdExtended($database, $RN_project_id);
	...
	$costCodeDivider = CostCodeDividerForUserCompany::getCostCodeDividerForUserCompanyById($database, $RN_user_company_id);

	$arrGcBudgetLineItemsByProjectId = GcBudgetLineItem::loadGcBudgetLineItemsByProjectId($database, $RN_project_id, $loadGcBudgetLineItemsByProjectIdOptions);

	$costCodeDivisionCnt = GcBudgetLineItem::costCodeDivisionCountByProjectId($database, $RN_project_id);

	$arrAllSubcontractorBidStatuses = SubcontractorBidStatus::loadAllSubcontractorBidStatuses($database);

	$main_company = Project::ProjectsMainCompanyApi($database, $RN_project_id);

	$arrSubcontractorBidStatusCountsByProjectId = SubcontractorBid::loadSubcontractorBidStatusCountsByProjectId($database, $main_company, $RN_project_id, $cost_code_division_id_filter);
	...
	$gcBudgetLineSiteworksVar = $gcBudgetLinebuildingcostVar = $gcBudgetLinesoftcostVar =$gcBudgetLineGeneralcondVar =  $gcBudgetLineSiteworksOrg = $gcBudgetLinebuildingcostOrg = $gcBudgetLinesoftcostOrg =$gcBudgetLineGeneralcondOrg = $csfunit_GeneralcondVar = $csfunit_SiteworksVar = $csfunit_buildingcostVar  = $csfunit_softcostVar =  $sav_GeneralcondVar = $sav_SiteworksVar = $sav_buildingcostVar  = $sav_softcostVar = $OverallOrg  = $overall_sav = $overall_var = $overall_csfunit = $cpsfunit_GeneralcondVar = $cpsfunit_SiteworksVar = $cpsfunit_buildingcostVar  = $cpsfunit_softcostVar = $overall_cpsfunit = 0;
	...
	foreach ($arrGcBudgetLineItemsByProjectId as $gc_budget_line_item_id => $gcBudgetLineItem) {
		...
		$reallocated_amt = ChangeOrder::ReallocationInDrawsAndCOByCostCode($database,$cost_code_id,$RN_project_id);
		...
		$ocoVal = ChangeOrder::loadSumOfCostCodeBreakDownByProjectIdAndCostCodeId($database, $cost_code_id,$RN_project_id);
		...
		// Reallocation value
		$reallocationVal = DrawItems::costcodeReallocated($database, $cost_code_id,$RN_project_id);
...
/*	   		$subtable = GenActualOriginalValue($cost_code_id,$project_id,$tmp_subcontract_id);

			$subcontract_actual_value +=$subtable;*/
			//To check the SCO Exists for the subcontarctor
			$resdata =SubcontractChangeOrderDataAjaxApi($database, $cost_code_id, $RN_project_id, "all", $gc_budget_line_item_id, $tmp_subcontract_id);


			$tmp_subcontract_actual_value =$tmp_subcontract_actual_value;
		...
		//$v_raw = $gcBudgetLineItemVariance = $pcsv - ($forecast + $sav);
	...
	$gcBudgetsubLineItems_arr['cost_code'] = ' '.$costCodeDivision->escaped_division_number.$costCodeDivider.$costCode->escaped_cost_code;
...
foreach($gcBudgetLineGeneralcond as $eachlineitem){
	...
	/* Sub Total - Start */
		...
		if(isset($eachlineitem['subtotal_array']) && $includesubTotal == "true"){
			$index++;
...
/* Total Soft cost - Start */
	...
	if ($OCODisplay == 1) {
		...
		$alphaCharIn = 0;
		/* COR function - start */
		//$primeContractScheduledValueTotal = $totalPrimeSchedule;
		$db = DBI::getInstance($database);
	    $db->free_result();
		...
		$totalCORs = ChangeOrder::loadChangeOrdersCountByProjectId($database, $RN_project_id, $loadChangeOrdersByProjectIdOptions);
