...
$arrSubmittals = Submittal::loadSubmittalsByProjectIdAndCostCode($database, $RN_project_id, $loadSubmittalsByProjectIdOptions,$RN_reportsStartDate,$RN_reportsEndDate);
...
foreach ($arrSubmittals as $submittal_id => $submittal) {
    ...
    // if (($submittalId == 2) && ($su_closed_date <> '0000-00-00') || ($submittalId == 3) && ($su_closed_date <> '0000-00-00')) {
    ...
    $alphaCharIn++;
    $objPHPExcel->getActiveSheet()->setCellValue($alphas[$alphaCharIn].$index,$formattedSuCostCode);
    $alphaCharIn++;
