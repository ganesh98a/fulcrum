...
if (isset($project_id) && !empty($project_id)) {
	...
	$project_id = (int) $session->getCurrentlySelectedProjectId();
}
	...
	case 'Submittals__loadCreateSuDialog':
		...
		try {
			...
			if ($submittal_draft_id) {
				...
				$submittalDraft = SubmittalDraft::findSubmittalDraftByIdExtended($database, $submittal_draft_id);
				$htmlContent = buildCreateSuDialog($database, $user_company_id, $project_id, $currentlyActiveContactId, null, $submittalDraft);
			} else {
				$htmlContent = buildCreateSuDialog($database, $user_company_id, $project_id, $currentlyActiveContactId);
			}
