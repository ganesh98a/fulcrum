...
$htmlJavaScriptBody .= <<<END_HTML_JAVASCRIPT_BODY
  ...
  <script src="/js/modules-retention-draw-list.js" ></script>
...
$htmlTitle = 'Retention Draws - '.$currentlySelectedProjectName;
$htmlBody = '';
...
$applicationId = $query['retentionId'];
// generate signature blocks

$createDrawForm = renderCreateRetentionHtml($database,$project_id,$applicationId);
$retention_draw_id = RetentionDraws::findLastRetentionIdUsingAppId($database, $project_id, $applicationId);
updateProjectOwnerContractor($database, $project_id, $retention_draw_id,'Retention');
$signaterwDrawContent = renderRetentionSignatureBlock($database, $project_id, $retention_draw_id,'Retention');
...
$template->assign('drawId', $retention_draw_id);
$template->assign('softwareModuleFunctionLabel','Retention Draws');
$template->assign('type','Retention');
