...
class Permissions extends IntegratedMapper
{
	/**
	...
	private $arrNavActive; // Default Active Nav
	...
	public function getArrNavActive()
	{
		if (isset($this->arrNavActive)) {
			$arrNav = $this->arrNavActive;
		} else {
			$arrNav = array();
		}
	...
	public function setArrNavActive($arrNavActive)
	{
		$this->arrNavActive = $arrNavActive;
	}
...
//AND smf.`show_in_navigation_flag` = 'Y'
	...
	public static function loadPermissions($database)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
		...
		if ($userRole == "global_admin") {
			...
			$arrGlobalAdminSoftwareModuleFunctionIds = Permissions::loadGlobalAdminSoftwareModuleFunctionIds($database);
		} else {
			// Not a Global Admin case

			// "All Users Nav"
			// Load permissions available to all users based on smf flags
			$arrAllUsersSoftwareModuleFunctionIds = Permissions::loadAvailableToAllUsersSoftwareModuleFunctionIds($database);
			...
			if ($userRole == 'admin') {
				...
				$arrAdminNonProjectSoftwareModuleFunctionIds = Permissions::loadAdminNonProjectSoftwareModuleFunctionIds($database, $user_company_id);

				// Project-specific
				// sm.`project_specific_flag` = 'Y'
				if ($currentlySelectedProjectUserCompanyId == $user_company_id) {
					$arrAdminProjectOwnerSoftwareModuleFunctionIds =
						Permissions::loadAdminProjectOwnerSoftwareModuleFunctionIds($database, $user_company_id, $currentlySelectedProjectId);
				}
			}
			...
			if ($userRole == 'user') {
				...
				$arrUserNonProjectRoleBasedSoftwareModuleFunctionIds =
					Permissions::loadUserNonProjectRoleBasedSoftwareModuleFunctionIds($database, $primary_contact_id, $user_company_id);

				// By Contact, non-project-specific
				// contacts_to_software_module_functions
				$arrUserNonProjectContactBasedSoftwareModuleFunctionIds =
					Permissions::loadUserNonProjectContactBasedSoftwareModuleFunctionIds($database, $primary_contact_id, $user_company_id);
				...
				$arrUserNonProjectAllUsersBasedSoftwareModuleFunctionIds =
					Permissions::loadUserNonProjectAllUsersBasedSoftwareModuleFunctionIds($database, $user_company_id);
				...
				if ($currentlySelectedProjectUserCompanyId == $user_company_id) {
					...
					$arrUserProjectSpecificRoleBasedSoftwareModuleFunctionIds =
						Permissions::loadUserProjectSpecificRoleBasedSoftwareModuleFunctionIds($database, $primary_contact_id, $user_company_id, $currentlySelectedProjectId);

					// By Contact, project-specific
					// projects_to_contacts_to_software_module_functions
					$arrUserProjectSpecificContactBasedSoftwareModuleFunctionIds =
						Permissions::loadUserProjectSpecificContactBasedSoftwareModuleFunctionIds($database, $primary_contact_id, $user_company_id, $currentlySelectedProjectId);
					...
					$arrUserProjectSpecificAllUsersBasedSoftwareModuleFunctionIds =
						Permissions::loadUserProjectSpecificAllUsersBasedSoftwareModuleFunctionIds($database, $primary_contact_id, $user_company_id, $currentlySelectedProjectId);
				}
			}
			...
			$csvContactIds = Permissions::loadGuestContactIdsCSV($database, $user_company_id, $user_id, $primary_contact_id);

			// By Role, non-project-specific
			// contacts_to_roles, roles_to_software_module_functions
			//$guestNonProjectRoleIdCSV = Permissions::loadGuestNonProjectRoleIdsCSV($database, $csvContactIds);
			$arrGuestNonProjectRoleBasedSoftwareModuleFunctionIds =
				Permissions::loadGuestNonProjectRoleBasedSoftwareModuleFunctionIds($database, $csvContactIds, $currentlySelectedProjectUserCompanyId);

			// By Contact, non-project-specific
			// contacts_to_software_module_functions
			$arrGuestNonProjectContactBasedSoftwareModuleFunctionIds =
				Permissions::loadGuestNonProjectContactBasedSoftwareModuleFunctionIds($database, $csvContactIds, $currentlySelectedProjectUserCompanyId);

			// By "All Users", non-project-specific
			// user_companies_to_all_users_to_software_module_functions w/ employee_only_flag='N', sm.project_specific_flag=N
			$arrGuestNonProjectAllUsersBasedSoftwareModuleFunctionIds =
				Permissions::loadGuestNonProjectAllUsersBasedSoftwareModuleFunctionIds($database, $currentlySelectedProjectUserCompanyId);

			// By Role, project-specific
			// projects_to_contacts_to_roles, projects_to_roles_to_software_module_functions
			$arrGuestProjectSpecificRoleBasedSoftwareModuleFunctionIds =
				Permissions::loadGuestProjectSpecificRoleBasedSoftwareModuleFunctionIds($database, $csvContactIds, $currentlySelectedProjectUserCompanyId, $currentlySelectedProjectId);

			// By Contact, project-specific
			// projects_to_contacts_to_software_module_functions
			$arrGuestProjectSpecificContactBasedSoftwareModuleFunctionIds =
				Permissions::loadGuestProjectSpecificContactBasedSoftwareModuleFunctionIds($database, $csvContactIds, $currentlySelectedProjectUserCompanyId, $currentlySelectedProjectId);

			// By "All Users", project-specific
			// user_companies_to_all_users_to_software_module_functions w/ employee_only_flag='N', sm.project_specific_flag=Y
			$arrGuestProjectSpecificAllUsersBasedSoftwareModuleFunctionIds =
				Permissions::loadGuestProjectSpecificAllUsersBasedSoftwareModuleFunctionIds($database, $csvContactIds, $currentlySelectedProjectUserCompanyId, $currentlySelectedProjectId);
		}

			// Software module selected for the company according to new permission
			
			$arrsoftwaremodulefunctionToUserCompanies =
				Permissions::loadSoftwareModuleBasedModulesSelectedInCompany($database, $user_company_id);
		...
		//To restrict the modules that selected in the company level
		$arrSoftwareModuleFunctionIds = $arrsoftwaremodulefunctionToUserCompanies;

		// echo "<pre>";
		// print_r($arrGlobalAdminSoftwareModuleFunctionIds);

		$UserRole = $session->getUserRole();

		$arrTmp = array_keys($arrSoftwareModuleFunctionIds);
		$arrReturn = Permissions::loadPermissionsById($database, $arrTmp,$UserRole);

		/**
		 * @todo Delete $arrNav
		 */
		$arrNav = $arrReturn['nav'];
		$arrNavActive = $arrReturn['navActive'];		
		...
		$permissions->setArrNavActive($arrNavActive);
	...
	public static function loadPermissionsById($database, $arrId,$actualUserRole)
	{
		...
		$que="SELECT * FROM `software_module_functions`
WHERE `software_module_function_navigation_label` IN ('Dashboard','Manage Role & Permission','Impersonate Users','Manage Registered Users','Manage GA Permissions') OR `software_module_function_label`='Manage Registered Companies (User Companies)'";
		$db->query($que);
...
WHERE smf.`software_module_id` = sm.`id`  AND  sm.`web_view_flag` = 'Y' 
	AND sm.`software_module_category_id` = smc.`id`
	AND smf.`id` IN ($in) AND sm.`disabled_flag` = 'N'
...
";
		...
		while ($row = $db->fetch()) {
			...
			// if ($show_in_navigation_flag == 'Y') {
				...
				if (!isset($arrSoftwareModuleFunctionsGroupedByNavigationLabel[$software_module_function_navigation_label])) {
					...
					// set the path to activeNavArray
					$arrNavActive[$software_module_category_id][$software_module_function_id] =  $default_software_module_function_url;
				}
			// }
	 ...
	 */
	public static function loadGlobalAdminSoftwareModuleFunctionIds($database)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
";
	...
	public static function loadAvailableToAllUsersSoftwareModuleFunctionIds($database)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadAdminNonProjectSoftwareModuleFunctionIds($database, $user_company_id)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//INNER JOIN `software_module_categories` smc ON sm.`software_module_category_id` = smc.`id`
	 ...
	 * @return array of integer id values
	 */
	public static function loadAdminProjectOwnerSoftwareModuleFunctionIds($database, $user_company_id, $project_id)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	...
	//modules seleted in usercompany
	public static function loadSoftwareModuleBasedModulesSelectedInCompany($database,$user_company_id)
	{
		$session = Zend_Registry::get('session');
		$CurrentlySelectedProjectUserCompanyId = (int)$session->getCurrentlySelectedProjectUserCompanyId();
		$user_companies  = $user_company_id;
		if($CurrentlySelectedProjectUserCompanyId != $user_company_id)
		{
			$user_companies  .= ','.$CurrentlySelectedProjectUserCompanyId;
		}
		...
		INNER JOIN `software_modules` sm ON uc2sm.`software_module_id` = sm.`id`
		INNER JOIN `software_module_functions` smf ON sm.`id` = smf.`software_module_id`
		WHERE uc2sm.`user_company_id` IN ($user_companies)
		AND sm.`disabled_flag` = 'N'
		AND smf.`disabled_flag` = 'N'
		";
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadUserNonProjectRoleBasedSoftwareModuleFunctionIds($database, $csvContactIds, $user_company_id)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadUserNonProjectContactBasedSoftwareModuleFunctionIds($database, $csvContactIds)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadUserNonProjectAllUsersBasedSoftwareModuleFunctionIds($database, $currentlySelectedProjectUserCompanyId)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadUserProjectSpecificRoleBasedSoftwareModuleFunctionIds(
																							$database,
																							$csvContactIds,
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadUserProjectSpecificContactBasedSoftwareModuleFunctionIds($database, $csvContactIds, $currentlySelectedProjectUserCompanyId, $currentlySelectedProjectId)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadUserProjectSpecificAllUsersBasedSoftwareModuleFunctionIds($database, $currentlySelectedProjectUserCompanyId)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
";
	 ...
	 * @return array of integer id values
	 */
	public static function loadGuestNonProjectRoleBasedSoftwareModuleFunctionIds($database, $guestContactIdCSV, $currentlySelectedProjectUserCompanyId)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadGuestNonProjectContactBasedSoftwareModuleFunctionIds($database, $csvContactIds)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadGuestNonProjectAllUsersBasedSoftwareModuleFunctionIds($database, $currentlySelectedProjectUserCompanyId)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadGuestProjectSpecificRoleBasedSoftwareModuleFunctionIds(
																							$database,
																							$csvContactIds,
																							$currentlySelectedProjectUserCompanyId,
																							$currentlySelectedProjectId
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadGuestProjectSpecificContactBasedSoftwareModuleFunctionIds($database, $csvContactIds, $currentlySelectedProjectUserCompanyId, $currentlySelectedProjectId)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
//AND smf.`show_in_navigation_flag` = 'Y'
	 ...
	 * @return array of integer id values
	 */
	public static function loadGuestProjectSpecificAllUsersBasedSoftwareModuleFunctionIds($database, $currentlySelectedProjectUserCompanyId)
	{
		$db = DBI::getInstance($database);
		/* @var $db DBI_mysqli */
...
";
	...
	*/
	...
	public function checkDelayPermission($software_module_function)
	{
		...
		$arrContactPermissionsMatrix = Permissions::loadContactsForSoftwareModuleFunctionPermissionsMatrix($database, $user_company_id, $software_module_id, $software_module_functionId, $project_id);
