...
class CostCodeSubmittalRegistry extends IntegratedMapper
{
	/**
	 * Class name for use in deltifyAndSave().
	 */
	protected $_className = 'CostCodeSubmittalRegistry';
...
";
		...
		if ($row) {
			$cost_code_id = $row['id'];
			$costCode = self::instantiateOrm($database, 'CostCodeSubmittalRegistry', $row, null, $cost_code_id);
...
";
	...
	codes_fk_ccd.`sort_order` AS 'codes_fk_ccd__sort_order',
	codes_fk_ccd.`disabled_flag` AS 'codes_fk_ccd__disabled_flag',
	codes.* FROM `cost_codes` codes INNER JOIN `cost_code_divisions` codes_fk_ccd ON codes.`cost_code_division_id` = codes_fk_ccd.`id` WHERE codes.`id` IN ($csvCostCodeIds){$sqlOrderBy}{$sqlLimit} ";
