...
abstract class Zend_Captcha_Word extends Zend_Captcha_Base
{
    ...
    {
    	// Modified to work with Virtual Framework
     ...
     * @return string
     */
    protected function _generateWord($arrConsonantsToOmit=null, $arrVowelsToOmit=null)
    {
        $word       = '';
        ...
        for ($i=0; $i < $wordLen; $i = $i + 2) {
            ...
            } while (isset($arrConsonantsToOmit[$consonant]));

            do {
				$vowel = $vowels[array_rand($vowels)];
            } while (isset($arrVowelsToOmit[$vowel]));
     ...
     */
    public function generate($arrConsonantsToOmit=null, $arrVowelsToOmit=null)
    {
        $this->_session = null;
        $id             = $this->_generateRandomId();
        $this->_setId($id);
        $word           = $this->_generateWord($arrConsonantsToOmit, $arrVowelsToOmit);
        $this->_setWord($word);
        return $id;
    }
    ...
    {
    	$rand = mt_rand(0, 1000) . microtime(true);
    	$md5 = md5($rand);
        return $md5;
    }
